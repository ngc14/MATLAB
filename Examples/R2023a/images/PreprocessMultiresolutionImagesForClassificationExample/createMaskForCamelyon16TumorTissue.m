...
% The createMaskForCamelyon16TumorTissue function writes blockedImage masks
% at the specified resolution level, resolutionLevel for tumor regions in
% images in bims. The tumor regions (lesion annotations) are specified in
% the XML files in the directory annotationDir. The masks are written to
% disk at the specified location, maskDir.
%
...
for ind = 1:numel(bims)
    ...
    if isempty(roiXYs)
        bmasks(ind) = blockedImage(maskLocation,blockSize,imageSize,false,"Mode",'w',...
