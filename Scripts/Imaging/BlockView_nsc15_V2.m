function [] = BlockView_nsc15_V2(varargin)

%% user input
username = 'ngc14';                 % windows username

animal = 'Bordeaux';                  % animal name
hemi = 'Left';
date = '09_24_2018';                % experiment date
run = 'run01';

ALIGN_OFFSET = true;

% stim(1,1)={'Rest'};
% stim(3,1)={'Small Sphere'};
% stim(3,1)={'150BiPulses_60microAmps'};
% stim(4,1)={'150BiPulses_80microAmps'};
stim = containers.Map([0 1 2 3],{'Rest', 'Button','SmallSphere', 'LargeSphere'});

frameRange = [7:9]; %[16:17];
excludeFrames = []; %[5:12];
...
LPk=2;                     % Low-pass kernel size (usually small) set as 0 if no LP filter
HPMethod='fastmedian';      % High-pass filtering method, usually 'fastmedian'
HPk=200;                    % High-pass kernel size (usually greater than 250) set as 0 if no HP filter
...
numCols = 5;                % Number of columns in big image.
...
%% Begin Script
disp(['%%%%% BLOCKVIEW_NSC15_V2: ',animal,' ',date(date~='_'),' ',run,' [',stim(stim1),'] LP',...
...
maps_stim1(isinf(maps_stim1)) = NaN;
maps_stim2(isinf(maps_stim2)) = NaN;
...
imwrite(norm_to_uint8(big_map_noClip_small),gray(256),'AllFrames\gray-noClip.bmp');
...
if SAVE_IND_FRAMES
    ...
    for f = 1:NFrames
        if f<10
            imwrite(norm_to_uint8(maps_avg_indi(:,:,f)),gray(256),['Frame_0',num2str(f),'_clip_',num2str(clipValue),'.bmp']);
        else
            imwrite(norm_to_uint8(maps_avg_indi(:,:,f)),gray(256),['Frame_',num2str(f),'_clip_',num2str(clipValue),'.bmp']);
        end
    end
