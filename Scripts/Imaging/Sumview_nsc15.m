...
% m files called;  OIHeadRead.m, OIReadStim.m, OIClip.m, norm_to_uint8.m
...
datadriver = 'C:\Users\nsc15\Documents\Data\';     % Data disk name
outputdriver = 'C:\Users\nsc15\Documents\Data\';     % Data disk name
...
expname = [animal,'_SqM\',hemi,'Hemisphere\',date,'\']; % Exp folder name (in both data folder and result folder)
...
outfile='SmallSphere_.bmp';                  % part of output file name
...
style=4;    % 1: simple DC, 2:ffsub (need specify fframe), 3 subtraction (stim1 -stim2), no ffsub, 4: subtraction (stim1-stim2) with ffsub
fframe=[1];   % specify first frame (or reference frames), used in 'style=2' or 'style=4'
stim1=[2];  % specify stimulus list 1, used in all styles, if more than one stim, will take average
stim2=[1];  % specify stimulus list 2, used in style=3 and style=4, if more than one stim, will take average
...
% stims{2} = 'Blue LED 500ms';
% stims{3} = 'Blue LED 1000ms';
% stims{4} = 'Blue LED 1500ms';
% stims{3} = 'Blue LED Train';
...
HPKernel=0; %100;             % For 'ribot' filter, it's the order of polynomial fit, usually set as 2, 
...
clipMethod = 2;
sd=2;       % number of standard deviations in map clipping
...
if isempty(dir([blockfolder,'*.blk']))
...
if clipMethod~=2
    ...
    clipMask = imread(['C:\Users\nsc15\Documents\Data\',animal,'_SqM\',hemi,'Hemisphere\',...
        date,'\',run,'\clipMask',run(end-1:end),'.bmp']);
...
mapsAll = zeros(Height, Width, NFrames, blockfilenum);
...
for k=1:blockfilenum
    ...
    for j=1:size(stim1,2)   % read file
        Frames(:,:,:,j)=OIReadStim(strcat(blockfolder, blkfilename), stim1(j), system);
    end
	if style==3||style==4
        for j=1:size(stim2,2)
            Frames2(:,:,:,j)=OIReadStim(strcat(blockfolder, blkfilename), stim2(j), system);
        end
    end
		...
		for i=1:rownum %col
            for j=1:colnum  %row
                ...
                if framenum<=NFrames
                    ...
                    bigframe(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClipH(singlemaps(:,:,framenum),clipMethod,sd,clipMask));
                    ...
                    case 2
                        if size(fframe,2)==1 && framenum==fframe
                            ...
                            bigframe(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(singlemaps(:,:,framenum),1,sd));
                        end
                    case 3
                        bigframe(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(singlemaps(:,:,framenum),1,sd));
                    case 4
                    ...
                    if flagtrialfilter==1
                        ...
                        case 2
                            if size(fframe,2)==1 && framenum==fframe
                             ...
                             bigframef(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(singlemapsf(:,:,framenum),1,sd));
                            end
                        case 3
                            bigframef(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(singlemapsf(:,:,framenum),1,sd));
                        case 4
...
maps = maps./blockfilenum;
mapsSTD = std(mapsAll,0,4);
if flagtrialfilter==1
    mapsf=mapsf./blockfilenum;   
end 

mapsZ = zeros(Height,Width,NFrames,blockfilenum);
for f = 1:NFrames
    for n = 1:blockfilenum
...
for i=1:rownum %col
    for j=1:colnum  %row
        ...
        if framenum<=NFrames
            ...
            switch style
            case 1
                bigframe(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(maps(:,:,framenum),1,sd));
            case 2
                if size(fframe,2)==1 && framenum==fframe
                else
                    bigframe(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(maps(:,:,framenum),1,sd));
            ...
            if flagtrialfilter==1
                switch style
                case 1
                    bigframef(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(mapsf(:,:,framenum),1,sd));
                case 2
                    if size(fframe,2)==1 && framenum==fframe
                    else
                        bigframef(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(mapsf(:,:,framenum),1,sd));
            ...
            if flagsumfilter==1
                ...
                switch style
                case 1
                    bigframefs(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(mapsfs(framenum),1,sd));
                case 2
                    if size(fframe,2)==1 && framenum==fframe
                    else
                        bigframefs(y:y+Height-1, x:x+Width-1)=norm_to_uint8(OIClip(mapsfs(framenum),1,sd));
...
%else
%    bigframe(end-Height+1-spacepixy:end-spacepixy, end-Width+1-spacepixx:end-spacepixx)=norm_to_uint8(OIClip(mean(maps, 3),1,sd));
%end
