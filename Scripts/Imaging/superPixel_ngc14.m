function [] = superPixel_ngc14(varargin)
% close all;
...
hemi = 'Left';
date = '11_23_2020';                % experiment date
run = 'run00';                      % run name
...
RIGID = false;
HPk = 250;                          % High pass filter kernel
HPMethod = 'fastmedian';            % High pass filter method
LPk = 5;                           % Low pass filter kernel
LPMethod = 'gaussian';              % Low pass filter method

CLIP = false;
clipMethod = 6;                     % Clip Method
...
stims = containers.Map([0 2 4 5],{'Blank', 'ExtraSmallSphgere', 'LargeSphere', 'Photocell'});
...
if ~isempty(varargin)
    ...
    if(strcmp(date, '11_09_2020') || strcmp(date, '12_09_2020'))
        stims = containers.Map([0 4 5],{'Blank', 'LargeSphere', 'Photocell'});
    end
...
% data path
path = ['S:\Lab\',monkey,'\All Data\', monkey, '_',date, '\Imaging\',run];
...
clipMask = ~(~clipMask | imgaussfilt(greenGray,1) < median(greenGray(:))-0.1*std(greenGray(:)));
...
if USE_MASKS
    if(RIGID)
        ...
        tform = load('tform_nonrigid.mat');
        clipMask = bspline_transform(tform.O_trans,clipMask,tform.Spacing,3);
        
    end
    clipMask = logical(clipMask);
    if ~GRID_MASKS
        cd(['S:\Lab\',monkey,'\Mapping\SPMasks\']);
                        maskFiles = dir('*.bmp');
        ...
        for i = 1:length(maskFiles)
            mask{i} = imread(fileNames{i});
            mask{i} = mask{i}(:,:,1)>200;
            if size(mask{i},1) == 1082
                mask{i} = mask{i}(2:end-1,:);
            end
            if size(mask{i},2) == 1312
                mask{i} = mask{i}(:,2:end-3);
            end
            if size(mask{i},2)==1310
               mask{i} = mask{i}(:,2:end-1); 
            end
    %         %             if(size(mask{i},1)>768)
    %         %                 mask{i}(769,:) = [];
    %         %             end
    %         %             if(size(mask{i},2)>768)
    %         %                 mask{i}(:,769) = [];
    %         %             end
    %         %             CC = bwconncomp(mask{i});
    %         %             RP = regionprops(CC);
    %         %             [~, maxIdx] = min([RP.Area]);
    %         %             eraseIdx = 1:length(CC.PixelIdxList);
    %         %             eraseIdx(maxIdx) = [];
    %         %             mask{i}(vertcat(CC.PixelIdxList{eraseIdx})) = 0;
            mask{i} = mask{i} & clipMask;
        end
    cd(path);
    ...
    if GRID_MASKS
        ...
        for m = 1:ceil(H/maskSpacing)
            for n = 1:ceil(W/maskSpacing)
                mask{count} = zeros(H,W);
                mask{count}(((m-1) * maskSpacing) + 1:min((m * maskSpacing) + 1,H) , ((n-1) * maskSpacing) + 1:min((n * maskSpacing) + 1 ,W)) = 1;
                mask{count}(refMask==1) = 0;
                count = count + 1;
            end
        end
    end
...
for s = 1:length(stimSelect)
    ...
    if(s==1)
        alignInd1 = find(cellfun(@(a) a==0, stims.keys));
    ...
    NBlocks = min(NBlocks,min(size(blocksCond,1), size(blankBlocks,1)));
    ...
    for n = 1:NBlocks
        ...
        frames(:,:,:) = OIReadStim([blocksCond(n).folder,'\',blocksCond(n).name], 0, 'v');
        ...
        for f = fframe(1):fframe(end)
            frames(:,:,f) = OIShift(frames(:,:,f),-1*round(align.disp_x_med{alignInd2}{n,f}),...
                -1*round(align.disp_y_med{alignInd2}{n,f}));
            if(RIGID)
                frames(:,:,f) = imwarp(frames(:,:,f), affine2d(tform),'OutputView',imref2d([W,H]));
            else
                frames(:,:,f) =  bspline_transform(tform.O_trans,frames(:,:,f),tform.Spacing,3);
            end
            if s==1
                frames_blank(:,:,f) = OIShift(frames_blank(:,:,f),-1*round(align.disp_x_med{alignInd1}{n,f}),...
                    -1*round(align.disp_y_med{alignInd1}{n,f}));
                if(RIGID)
                    frames_blank(:,:,f) = imwarp(frames_blank(:,:,f), affine2d(tform),'OutputView',imref2d([W,H]));
                else
                    frames_blank(:,:,f) =  bspline_transform(tform.O_trans,frames_blank(:,:,f),tform.Spacing,3);
                end
            end
        end
        
        for f = 1:NFrames
            if(~any(ismember(fframe, f)))
                frames(:,:,f) = OIShift(frames(:,:,f),-1*round(align.disp_x_med{alignInd2}{n,f}),...
                    -1*round(align.disp_y_med{alignInd2}{n,f}));
                if(RIGID)
                    frames(:,:,f) = imwarp(frames(:,:,f), affine2d(tform),'OutputView',imref2d([W,H]));
            ...
            if FFS
                framesFFS_blank{f}(:,:,n) = (frames_blank(:,:,f) - mean(frames_blank(:,:,fframe),3)) ./ mean(frames_blank(:,:,fframe),3) * 100;
                framesFFS{f}(:,:,n) = (frames(:,:,f) - nanmean(frames(:,:,fframe),3)) ./ nanmean(frames(:,:,fframe),3) * 100;
            else % no FFS
                framesFFS_blank{f}(:,:,n) = (frames_blank(:,:,f) - mean(frames_blank(:,:,fframe),3)) ./ mean(frames_blank(:,:,fframe),3) * 100;
                framesFFS{f}(:,:,n) = (frames(:,:,f) - nanmean(frames_blank(:,:,fframe),3)) ./ nanmean(frames_blank(:,:,fframe),3) * 100;
            end
        end
        
        if SP_INDI_BLKS  % filter individual blocks for superpixel
            %framesFFS_BS(:,:,f) = (framesFFS{f}(:,:,n)  - framesFFS_blank{f}(:,:,n));
            framesFFS_BS(:,:,f) = framesFFS{f}(:,:,n);
            if FILTER
                framesFFS_BS_FILT{s}{n}(:,:,f) = imageFilter_LPHP_nsc15(framesFFS_BS(:,:,f),LPk,HPk,[]);
            else
    ...
    clear frames framesFFS_BS;  % clear up some memory
    ...
    for f = 1:NFrames
        ...
        if FILTER
            framesFFS_AVG{s}(:,:,f) = imageFilter_LPHP_nsc15(framesFFS_AVG{s}(:,:,f),LPk,HPk,[]);
            framesFFS_blank_AVG(:,:,f) = imageFilter_LPHP_nsc15(framesFFS_blank_AVG(:,:,f),LPk,HPk,[]);
            framesFFS_AVG_BS_FILT{s}(:,:,f) = imageFilter_LPHP_nsc15(framesFFS_AVG_BS{s},LPk,HPk,[]);
        else
...
if SP_INDI_BLKS  % superpixel for each block
    ...
    for s = 1:length(stimSelect)
            ...
            if ~USE_MASKS  % if no masks, do superpixel at every pixel
                ...
                for m = 1:length(mask)
                    for f = 1:NFrames
                        ...
                        maskMean_BS{s}(m,f,n) = nanmean(tempData(mask{m}>0));
                        tempData = framesFFS_BS_FILT{s}{n}(:,:,f);
                        maskMean_blank(m,f,n) = nanmean(tempData(mask{m}>0));
                    end
                end
            end
        ...
        if USE_MASKS  % average mask superpixel over blocks
            maskMean_BS_Avg{s} = nanmean(maskMean_BS{s},3);
            maskMean_BS_Std{s} = nanstd(maskMean_BS{s},0,3)/sqrt(size(maskMean_BS{s},3));
...
for s = 1:length(stimSelect)
    if ~USE_MASKS
        ...
        maskMean_BS_Avg = [];
        if(~GRID_MASKS)
%             maskPath = ['S:\Lab\',monkey,'\Mapping\Clustering\15px_Grid\XCorr_Elbow\Biased\Norm_No_Clip\'];
%             mask =  {};
%             if (stimSelect(s)==2)
%                 %activationMask = imread(['S:\Lab\',monkey,'\Mapping\tTests\ESS_HSV.png']);
%                 %masksC = dir([maskPath,'ExtraSmallSphere_*.bmp']);
%                 clusters = openfig([maskPath, 'ExtraSmallSphere_Grid.fig'], 'invisible');
%             elseif(stimSelect(s)==4)
%                 %activationMask = imread(['S:\Lab\',monkey,'\Mapping\tTests\LS_HSV.png']);
%                 %masksC = dir([maskPath, 'LargeSphere_*.bmp']);
%                 clusters = openfig([maskPath, 'LargeSphere_Grid.fig'], 'invisible');
%                 
%             elseif(stimSelect(s)==5)
%                 %activationMask = imread(['S:\Lab\',monkey,'\Mapping\tTests\PC_HSV.png']);
%                 %masksC = dir([maskPath, 'Photocell_*.bmp']);
%                 clusters = openfig([maskPath, 'Photocell_Grid.fig'], 'invisible');
%                 
%             end
%             clusters = clusters.Children(1).Children(2).CData;
%             %fileNames = {masksC.name};
%             vessels = imread(['S:\Lab\',monkey,'\Mapping\clean_mask_filled.bmp']);
%             vessels = vessels(:,:,1) > 100;
%             vessels(:,1) = 0;
%             vessels(:,end-1:end) = [];
%             %             %             for m = 1:length(masksC)
%             %             %                 mask{m} = imresize(imread([maskPath,fileNames{m}]),[H, W]) & vessels;
%             %             %             end
%             for m = 1:max(max(clusters))
%                 mask{m} = imresize(clusters==m,[H, W]) & vessels;
%             end
%             %masksB = dir(['S:\Lab\',monkey,'\Mapping\Clustering\15px_Grid\XCorr_Elbow\Normalized\Rest_*.bmp']);
%             %blankMaskNames = {masksB.name};
%             masksB = openfig([maskPath, 'Rest_Grid.fig'], 'invisible');
%             masksB = masksB.Children(1).Children(2).CData;
%             for m = 1:max(max(masksB))
%                 maskBlank{m} = imresize(masksB==m,[H,W]) & vessels;
%             end
            %             %             for m = 1:length(masksB)
            %             %                 maskBlank{m} = imresize(imread(['S:\Lab\',monkey,'\Mapping\Clustering\15px_Grid\XCorr_Elbow\Normalized\',blankMaskNames{m}]),[H, W]) & vessels;
            %             %             end
            %
            %             %     activationMask = imresize(activationMask, [1080,1310]);
            %             %
            %             %     vessels = activationMask(:,:,1) < 100 & activationMask(:,:,2)<100 & activationMask(:,:,3)<100;
            %             %     activationMask = activationMask(:,:,1)>100 & activationMask(:,:,2)<100 & activationMask(:,:,3)<100;
            %             %     MM = double(imread(['S:\Lab\Skipper\Mapping\MM_G2S_RGB-01.png']));
            %             %
            %             %     %border is yellow
            ...
            %             %     PMpointsY = pointsY;
            %             %
            %             %     pointsX(end+1:end+2) = [1310, 1];
            %             %     pointsY(end+1:end+2) = [1 1];
            %             %     M1mask = double(poly2mask(pointsY,pointsX, 1080,1310));
            %             %
            %             %     PMpointsX(end+1:end+2) = [1310, 1];
            %             %     PMpointsY(end+1:end+2) = [1080 1310];
            %             %     PMmask = double(poly2mask(PMpointsY,PMpointsX, 1080,1310));
            %             %
            %             %     [row,col] = find(PMborder);
            %             %     row(end+1:end+2) = [1, 1];
            %             %     col(end+1:end+2) = [1310, min(col)];
            %             %     PMdmask = double(poly2mask(col,row,1080,1310));
            ...
            %             %     mask{1} = allMMs{1} & ~allMMs{2} & activationMask & M1mask;
            %             %     mask{2} = allMMs{2} & ~allMMs{1} & activationMask & M1mask;
            %             %     mask{3} = allMMs{3} & activationMask & M1mask;
            %             %     mask{4} = allMMs{4} & activationMask & M1mask;
            %             %     mask{5} = allMMs{1} & activationMask & PMdmask;
            %             %     mask{6} = allMMs{2} & activationMask & PMdmask;
            %             %     mask{7} = allMMs{3} & activationMask & PMdmask;
            %             %     mask{8} = allMMs{4} & activationMask & PMdmask;
            %             %     mask{9} = allMMs{1} & ~allMMs{2} & ~activationMask & M1mask & ~vessels;
            %             %     mask{10} = allMMs{2} & ~allMMs{1} & ~activationMask & M1mask & ~vessels;
            %             %     mask{11} = allMMs{3} & ~activationMask & M1mask & ~vessels;
            %             %     mask{12} = allMMs{4} & ~activationMask & M1mask & ~vessels;
            %             %     mask{13} = allMMs{1} & ~activationMask & PMdmask & ~vessels;
            %             %     mask{14} = allMMs{2} & ~activationMask & PMdmask & ~vessels;
            %             %     mask{15} = allMMs{3} & ~activationMask & PMdmask & ~vessels;
            %             %     mask{16} = allMMs{4} & ~activationMask & PMdmask & ~vessels;
            %             %     mask{17} = allMMs{1} & ~allMMs{2} & M1mask & ~vessels;
            %             %     mask{18} = allMMs{2} & ~allMMs{1} & M1mask & ~vessels;
            %             %     mask{19} = allMMs{3} & M1mask & ~vessels;
            %             %     mask{20} = allMMs{4} & M1mask & ~vessels;
            %             %     mask{21} = allMMs{1} & PMdmask & ~vessels;
            %             %     mask{22} = allMMs{2} & PMdmask & ~vessels;
            %             %     mask{23} = allMMs{3} & PMdmask & ~vessels;
            %             %     mask{24} = allMMs{4} & PMdmask & ~vessels;
            %             %     mask{25} = armArea & ~handArea & activationMask & ~vessels;
            %             %     mask{26} = handArea & ~armArea & activationMask & ~vessels;
            %             %     mask{27} = (armArea | handArea) & M1mask & activationMask & ~vessels;
            %             %     mask{28} = (armArea | handArea) & PMdmask & activationMask & ~vessels;
...
%             masks(end+1) = {mask};
        ...
        for m = 1:length(mask)
            for f = 1:NFrames
                ...
                maskMean_stim_Avg(m,f) = nanmean(tempData2(mask{m}>0));
                
                tempData2 = framesFFS_blank_AVG(:,:,f);
                maskMean_blank_Avg(m,f) = nanmean(tempData2(mask{m}>0));
                
                tempData2 = framesFFS_AVG_BS_FILT{s}(:,:,f);
                maskMean_BS_Avg(m,f) = nanmean(tempData2(mask{m}>0));
            end
        ...
        maskMean_stim_Avg2{s} = maskMean_stim_Avg;
        maskMean_blank_Avg2{s} = maskMean_blank_Avg;
        maskMean_BS_Avg2{s} = maskMean_BS_Avg;
        
        
%         maskMean_blankAvg2{s} = [];
%         maskMean_blank_Avg = [];
%         
%         for m = 1:length(maskBlank)
%             for f = 1:NFrames
%                 tempData2 = framesFFS_blank_AVG(:,:,f);
%                 maskMean_blank_Avg(m,f) = nanmean(tempData2(maskBlank{m}>0));
%             end
%         end
%         maskMean_blank_Avg2{s} = maskMean_blank_Avg;
            ...
            if SP_INDI_BLKS
                ...
                for i = 1:size(maskMean_BS_Avg{s},1)
                    ax = shadedErrorBar(xtime,maskMean_BS_Avg{s}(i,:),maskMean_BS_Std{s}(i,:),...
                        'lineprops',{'.-','color',colors(i,:),'linewidth',2});
...
if(0)
    ...
    for s =1:length(stimSelect)
        ...
        %     sigInds_stim(:,1:25) = 0;
        %     [~, sigFrame_stim] = max(maskMean_stim_Avg2{s}, [],2);
        ...
        set(gca,'fontsize',14);
        if FFS, ylim([-0.2 0.1]); end
        title(['[',stims(stimSelect(s)),'] - [',stims(0),']']);
        ...
        for m = 1:size(maskMean_BS_Avg2{s},1)
            ...
            if m==1
                legend([{stims(0)}, {stims(stimSelect(s))}, {[stims(stimSelect(s)), '-', stims(0)]}]);
            end
...
if SAVE && USE_MASKS
    ...
    if(SP_INDI_BLKS)
        ...
        SPData.maskMeanAll = maskMean_BS;
        SPData.maskMeanBlank = maskMean_blank;
    end
    SPData.maskMean_BS_Avg2 = maskMean_BS_Avg2;
    SPData.mask = masks;
    SPData.maskMean_stim_Avg2 = maskMean_stim_Avg2;
    SPData.maskMean_blank_Avg2 = maskMean_blank_Avg2;
    
    if GRID_MASKS
        if GRID_MASKS_INVERT
            save('SPData_GridMasks_Invert.mat','SPData','-v7.3');
        else
            save('SPData_GridMasks.mat','SPData','-v7.3');
        end
    else
        %save(['SPData_',num2str(stimSelect(1)), '.mat'],'SPData','-v7.3');
        save('SPData.mat', 'SPData', '-v7.3');
    end
    cd(parent);
end
