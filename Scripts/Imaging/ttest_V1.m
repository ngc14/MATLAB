...
LPk=4;                     % Low-pass kernel size (usually small) set as 0 if no LP filter
...
parfor n = 1:NBlocks
    ...
    % load frames
    frames_all_stim1(:,:,:) = OIReadStim([blockConds1(n).folder,'\',blockConds1(n).name],1,'v');
    frames_all_stim2(:,:,:) = OIReadStim([blockConds2(n).folder,'\',blockConds2(n).name],1,'v');
...
%             disp(['n',num2str(n),' f',num2str(f)]);
    ...
    frames_avg_stim2(:,:,n) = imageFilter_LPHP_nsc15(frames_avg_stim2(:,:,n), LPk, HPk, clipMask);
...
subMapC = norm_to_uint8(OIClipH(subMap,clipMethod,clipValue,clipMask));

[h, p] = ttest2(frames_avg_stim1,frames_avg_stim2,0.05,'right','equal',3);
...
for i = 1:length(p_values)
    imwrite(p < p_values(i) & clipMask,[stimName,'_p-',num2str(p_values(i)),'.bmp']);
    imwrite(ones(W,H,3).*cat(3,1,0,0),[stimName,'_p-',num2str(p_values(i)),'.png'],...
        'alpha',double((p < p_values(i) & clipMask)));
end
