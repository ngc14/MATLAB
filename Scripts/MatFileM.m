...
classdef (Sealed) MatFileM < dynamicprops
    ...
    %      Properties.Writable      - Whether to allow saving to the file.
    ...
    % end
    % methods
    %     function propval = get.varInfo(obj)
    %         propval = obj.varInfo;
    %     end
    %     function set.varInfo(obj,val)
    %         obj.varInfo = val;
    %     end
    ...
    methods(Access = private, Hidden = true)
        ...
        function varargout = genericWho(obj, fcnHan, fcnName, varargin)
            ...
            for k = find(ismember(varargin,fields(obj.varInfo)))
                varargout{k} = obj.varInfo.(varargin{k});
            end
            if isempty(varargin)||any(~ismember(varargin,fields(obj.varInfo)))
                if ~sourceExists(obj)
                    ...
                    if isempty(~ismember(varargin,fields(obj.varInfo)))
                        inds = 1:nargout;
                    else
                        inds =  ~ismember(varargin,fields(obj.varInfo));
                    end
                    [varargout{inds}] = fcnHan( ...
                        '-file', obj.Properties.Source, ...
                        '-protectedLoading', obj.Properties.ProtectedLoading, ...
                        varargin{~ismember(varargin,fields(obj.varInfo))});
                end
                obj.varInfo = varargout{1};
            end
        end
    ...
    methods(Access = public, Hidden = true)
        ...
        function varargout = who(obj, varargin)
            ...
            %   DETAILS = WHO(MATOBJ) lists all variables in the MAT-file
            %   associated with MATOBJ. DETAILS is a cell array of variable
            %   name strings.
            %
            %   DETAILS = WHO(MATOBJ,VARIABLES) lists the specified
            %   variables. Use one of these forms for VARIABLES:
            %
            %      VAR1,...,VARN    Comma-separated list of variable name
            %                       strings. Use the '*' wildcard to match
            %                       patterns. For example, who(MATOBJ,'A*')
        ...
        function obj = MatFileM(Source, varargin)
            narginchk(1, inf);
            ...
            parseInputs.addParameter('ProtectedLoading', ...
                settings().matlab.general.matfile.ProtectedLoading.ActiveValue);
            parseInputs.parse(varargin{:});
            try
                obj.Properties.Writable = parseInputs.Results.Writable;
                obj.Properties.ProtectedLoading = ...
                    parseInputs.Results.ProtectedLoading;
            catch caughtException
                throw(caughtException);
            end
            vNames = string(who('-file',obj.Properties.Source));
            if(~any(contains(vNames,"varInfo")))
                varInfo = whos('-file',obj.Properties.Source);
                save(obj.Properties.Source,'varInfo','-append');
                obj.varInfo = varInfo;
            else
                varInfo = load(obj.Properties.Source,'varInfo');
                obj.varInfo = varInfo.varInfo;
            end
            varInfo = obj.varInfo;
        ...
        function varargout = subsref(obj, indexingStruct)
            ...
            try
                if strcmp(varName,'Properties')
                    ...
                    vI = getVariableInfoIfItExistsInSource(obj, varName);
                    switch length(indexingStruct)
                        ...
                        case 2
                            ...
                            checkForOverriddenIndexing(vI, 'subsref');
                            ...
                            varSubset = createMetaDescriptionOfIndexing(varName, vI, indexingStruct);
                ...
                if strcmp(varName,'Properties')
                    ...
                    switch length(indexingStruct)
                        ...
                        case 2
                            ...
                            vI = whos(obj, varName);
                            checkForOverriddenIndexing(vI, 'subsasgn');
                            varSubset = createMetaDescriptionOfIndexing(varName, vI, indexingStruct);
        ...
        function disp(obj)
            ...
            allNames = strjust(char([propNamesLabels; {vI.name}']), 'right');
            rows = size(allNames,1);
            lhs = [repmat(' ',rows,4) strcat(allNames,':') repmat(' ',rows,1)];

            % Get size and type of properties and variables
            propValues = cell(length(propNames),1);
            for i = 1:length(propNames)
                propValues{i} = mat2str(obj.Properties.(propNames{i}));
            end
            varSize = cellfun(@sizeToString, {vI.size}, 'UniformOutput', false);

            % Generate display of values "RHS"
            if ~isempty(varSize)
                sizeAndTypeStr = [char(varSize), repmat(' ',length(vI),1),...
                    char({vI.class})];
...
try
    sourceName = matlab.io.internal.findFullMATFilename(sourceName);
catch caughtException
    throwAsCaller(caughtException);
end
...
if ~ischar(indexingStruct(1).subs) && ~isStringScalar(indexingStruct(1).subs)
...
% LocalWords:  MATOBJ matfile ISWRITABLE VARNAME SMALLERVAR SIZEMYVAR myfile
% LocalWords:  nrows ncols durer mycopy subsassigndimmismatch Doesnt VARN
% LocalWords:  ALLDIMS DIMN SELECTEDDIM mf Sqr Brkt isint badsubscript
