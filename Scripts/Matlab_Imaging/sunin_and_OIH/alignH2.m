function AlignH2(datadrive)

% AlignH2.m  Optical Imaging image alignment
%
% Ver 2.1.3: (150221:Hisashi) use OIAlignCoreH.m and make new output file "shiftinput.3.txt". change variable name 'pixoff' to 'pixshift'
% Ver 1:   (060417:HDL) modified from 'suninalign.m'
%
...
% This file calls 'OIAlignCoreH.m' which aligns two frames.
...
if manual==1;
    ...
    datadrive = 'K:\';     % Data disk name
    datafolder = 'expt0\';   % Data folder name on data disk, results will be saved in 'expresult'
    expname = '110404VitRG_AtnCon\'; % Exp folder name (in both data folder and result folder)
    ...
    baseframefile={'IMG_E01B000.BLK', 1, 1};  % filename, stim number, frame number for 'aligntype=2 or 4'
    ffrange=[1];      % the range of first frame to be aligned in 'aligntyp=2 or 4'
	framerange=[5:16];    % the range of frames to calculate correlations in the output
...
%     LPKernel=2;       % low-pass kernel, part of pre-process befor alignment, put 0 if no need 
	...
	HPKernel=50;             % For 'ribot' filter, it's the order of polynomial fit, usually set as 2, set as 0 if no HP filter
    ...
    crop1=[49 298 107 355];  % [x1, y1, x2, y2] of clip region, part of pre-process, put 0 if no need
...
%     percentage3=0.9;	% percent of trials to be included after shift3 process
    ...
    corrthreshold1=0.98; %Threshold for the correlation coefficient in shift2 process
    shiftthreshold1=10; %10 %Threshold for shift in pixel: if mean pixel shift for framerange exceeds this value, the trial is marked as '0'
    
    corrthreshold2=0.90; %Threshold for the correlation coefficient in shift3 process
    shiftthreshold2=6;%3; %Threshold for shift in pixel: if mean pixel shift for framerange exceeds this value, the trial is marked as '0'
end
% end of input %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     shiftlog1 = strcat(blockfolder, '_shiftlog', strcat(num2str(ctime(1)), '-', num2str(ctime(2)),'-',num2str(ctime(3)),'-',num2str(ctime(4)),'-',num2str(ctime(5))),'.2.txt');
	...
	fidshiftinput1=fopen(strcat('shiftinput.2.txt'), 'w');
...
%     shiftlog2 = strcat(blockfolder, '_shiftlog', strcat(num2str(ctime(1)), '-', num2str(ctime(2)),'-',num2str(ctime(3)),'-',num2str(ctime(4)),'-',num2str(ctime(5))),'.3.txt');
	...
	fidshiftinput2=fopen(strcat('shiftinput.3.txt'), 'w');

	fidshiftinput3=fopen(strcat('shiftinput.4.txt'), 'w');
end
...
%     imbase=OIAPreProcess(baseframe, baseillubk, LPKernel, [0 0], crop1);
    imbase=OIAPreProcessH(baseframe, LPMethod, LPKernel, HPMethod, HPKernel, [0 0], crop1);
else
        ...
        case 1  % General alignment
			...
			if dalsalineremove == 1
				for fr = 1:FramesPerStim  % 150221 HT
					 tDCFrames(:,:,fr) = OIDalsalineremover(tDCFrames(:,:,fr));
				end
			end           
...
%                 imshift=OIAPreProcess(tDCFrames(:,:,j), illubk, LPKernel, [0 0], crop1);
                imshift=OIAPreProcessH(tDCFrames(:,:,j), LPMethod, LPKernel, HPMethod, HPKernel, [0 0], crop1); % 150220 HT
                [junk, pixshift, bestobj, currentobj] = OIAlignCoreH(baseframe, imshift, shiftrange, objfun); % find the best alignment
                OIAWriteShiftLog(fidshiftlog, filename, k, i, j, pixshift(1), pixshift(2), bestobj, getfield(cell2struct(baseframefile(1), 'junk'), 'junk'), getfield(cell2struct(baseframefile(2), 'junk'), 'junk'), getfield(cell2struct(baseframefile(3), 'junk'), 'junk'), currentobj);
            end
...
%                 imshift=OIAPreProcess(tDCFrames(:,:,j), illubk, LPKernel, [0 0], crop1);
                imshift=OIAPreProcessH(tDCFrames(:,:,j), LPMethod, LPKernel, HPMethod, HPKernel, [0 0], crop1); % 150220 HT
                [imshifted, pixshift, bestobj, currentobj] = OIAlignCoreH(imbase, imshift, shiftrange, objfun);
                OIAWriteShiftLog(fidshiftlog, filename, k, i, j, pixshift(1), pixshift(2), bestobj, getfield(cell2struct(baseframefile(1), 'junk'), 'junk'), getfield(cell2struct(baseframefile(2), 'junk'), 'junk'), getfield(cell2struct(baseframefile(3), 'junk'), 'junk'), currentobj);
...
%             imfframe=OIAPreProcess(tDCFrames(:,:,fframe), illubk, LPKernel, [0 0], crop1);        % note: a bug here, fframe maybe more than one
            imfframe=OIAPreProcessH(tDCFrames(:,:,fframe), LPMethod, LPKernel, HPMethod, HPKernel, [0 0], crop1);        % note: a bug here, fframe maybe more than one % 150220 HT
...
%                 imshift=OIAPreProcess(tDCFrames(:,:,j), illubk, LPKernel, [0 0], crop1); % 150220 HT
                imshift=OIAPreProcessH(tDCFrames(:,:,j), LPMethod, LPKernel, HPMethod, HPKernel, [0 0], crop1);
                [junk, pixshift, bestobj, currentobj] = OIAlignCoreH(imfframe, imshift, shiftrange, objfun); 
                OIAWriteShiftLog(fidshiftlog, filename, k, i, j, pixshift(1), pixshift(2), bestobj, filename, i, 1, currentobj);
            end
...
%                 imshift=OIAPreProcess(tDCFrames(:,:,j), illubk, LPKernel, [0 0], crop1);
                imshift=OIAPreProcessH(tDCFrames(:,:,j), LPMethod, LPKernel, HPMethod, HPKernel, [0 0], crop1); % 150220 HT
                [imshifted, pixshiftff, bestobjff, currentobjff] = OIAlignCoreH(imbase, imshift, shiftrange, objfun);
                OIAWriteShiftLog(fidshiftlog1, filename, k, i, j, pixshiftff(1), pixshiftff(2), bestobjff, getfield(cell2struct(baseframefile(1), 'junk'), 'junk'), getfield(cell2struct(baseframefile(2), 'junk'), 'junk'), getfield(cell2struct(baseframefile(3), 'junk'), 'junk'), currentobjff);
%                 fprintf(fidshiftinput, '%s \t%d \t%d \t%d \t%3.1f \t%3.1f \t%6.6f\r\n', filename, k, i, j, pixshift(1), pixshift(2), bestobj);
                fprintf(fidshiftinput1, '%s \t%d \t%d \t%d \t%3.1f \t%3.1f \t%6.6f\r\n', filename, k, i, j, pixshiftff(1), pixshiftff(2), bestobjff);
            end

%             imfframe=OIAPreProcess(tDCFrames(:,:,fframe), illubk, LPKernel, [0 0], crop1);        % note: a bug here, fframe maybe more than one
            imfframe=OIAPreProcessH(tDCFrames(:,:,fframe), LPMethod, LPKernel, HPMethod, HPKernel, [0 0], crop1);        % note: a bug here, fframe maybe more than one % 150220 HT
            OIAWriteShiftLog(fidshiftlog2, filename, k, i, 1, 0, 0, 1, filename, i, 1, 1);          % just for consistency
			fprintf(fidshiftinput2, '%s \t%d \t%d \t%d \t%3.1f \t%3.1f \t%6.6f\r\n', filename, k, i, 1, 0, 0, 1); % just for consistency %150221 HT
			fprintf(fidshiftinput3, '%s \t%d \t%d \t%d \t%3.1f \t%3.1f \t%6.6f\r\n', filename, k, i, 1, pixshiftff(1), pixshiftff(2), bestobjff); % just for consistency %150221 HT
			
            for j = 2:FramesPerStim
%                 imshift=OIAPreProcess(tDCFrames(:,:,j), illubk, LPKernel, [0 0], crop1);
                imshift=OIAPreProcessH(tDCFrames(:,:,j), LPMethod, LPKernel, HPMethod, HPKernel, [0 0], crop1); % 150220 HT
                [junk, pixshift, bestobj, currentobj] = OIAlignCoreH(imfframe, imshift, shiftrange, 1);        % Note: here forced 'objfun' to 1
%                 OIAWriteShiftLog(fidshiftlog2, filename, k, i, j, pixshift(1), pixshift(2), bestobj, filename, i, 1, currentobj);
                OIAWriteShiftLog(fidshiftlog2, filename, k, i, j, pixshift(1), pixshift(2), bestobj, filename, i, 1, currentobj); % 150220 HT
                fprintf(fidshiftinput2, '%s \t%d \t%d \t%d \t%3.1f \t%3.1f \t%6.6f\r\n', filename, k, i, j, pixshift(1), pixshift(2), bestobj);
                fprintf(fidshiftinput3, '%s \t%d \t%d \t%d \t%3.1f \t%3.1f \t%6.6f\r\n', filename, k, i, j, pixshiftff(1)+pixshift(1), pixshiftff(2)+pixshift(2), bestobjff*bestobj);
            end
        otherwise
...
%             fidshiftinput= fopen('shiftinput.2.txt', 'a');
            fidshiftinput1= fopen('shiftinput.2.txt', 'a');
            fidshiftinput2= fopen('shiftinput.3.txt', 'a');
            fidshiftinput3= fopen('shiftinput.4.txt', 'a');
		end
    end % for i=1:NStim
end % for k1:blocknum
...
%     OIAshift2goodstim3(shiftlog2, percentage3, 1, NStim, FramesPerStim, [5:16]);
    OIAshift2goodstim3H(shiftlog2, corrthreshold2, shiftthreshold2, NStim, FramesPerStim, framerange);
case 4
    fclose(fidshiftlog1);
