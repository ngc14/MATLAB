...
if manual==1;
    ...
    datadrive = 'f:\';     % Data disk name
    datafolder = 'expt\';   % Data folder name on data disk, results will be saved in 'expresult'
    expname = '060922Jem\'; % Exp folder name (in both data folder and result folder)
    ...
    baseframefile={'Jem_E00B025.BLK', 1, 1};  % filename, stim number, frame number for 'aligntype=2 or 4'
    ...
    crop1=[260 18 378 233];  % [x1, y1, x2, y2] of clip region, part of pre-process, put 0 if no need
...
fidshiftinput=fopen(strcat(blockfolder, '_shiftinput-',  runname,'.', '.2.txt'), 'w');
...
baseframe=OIReadFrame(strcat(blockfolder, getfield(cell2struct(baseframefile(1), 'junk'), 'junk')), 'v', getfield(cell2struct(baseframefile(2), 'junk'), 'junk'), getfield(cell2struct(baseframefile(3), 'junk'), 'junk'));
