function AllFrames=OIReadStim_single(blkfilename, stim, system)

% 10/13/10 Hisashi: Modified to use single precision
...
if system=='v'
    ...
    case 12
	    ...
	    AllFrames=fread(fid,[Width*Height*NFrames],'uint16=>float32'); 
    case 13
	    fseek(fid, Width*Height*NFrames*(stim-1)*4, 'cof');
	    AllFrames=fread(fid,[Width*Height*NFrames],'uint32=>float32'); 
    case 14
	    fseek(fid, Width*Height*NFrames*(stim-1)*4, 'cof');
	    AllFrames=fread(fid,[Width*Height*NFrames],'float32=>float32'); 
    otherwise
