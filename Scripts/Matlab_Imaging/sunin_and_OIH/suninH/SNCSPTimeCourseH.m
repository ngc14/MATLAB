...
% %                 stimsumnum(i);
%                 
%                 SPcondtemp(:,:,SPcondnum+1:SPcondnum+stimsumnum(i))=squeeze(spvalues(:,:,i,1:stimsumnum(i)));
...
%             SPcondnum=0;
%             for i=condlist1
%                 SPcondsum=SPcondsum+squeeze(sum(spvaluesdrr(:,:,i,1:stimsumnum(i)),4));
%                 
% %                 SPcondtemp(:,:,n:n+stimsumnum(i)-1)=squeeze(spvaluesdrr(:,:,i,1:stimsumnum(i)));
%                 SPcondtemp(:,:,SPcondnum+1:SPcondnum+stimsumnum(i))=squeeze(spvaluesdrr(:,:,i,1:stimsumnum(i)));
%                 SPcondnum=SPcondnum+stimsumnum(i);
%     %             n=n+stimsumnum(i);
%             end
%             stimsumnum(j)=SPcondnum;
%             if SPcondnum>0 %n>1
%                 spavgdrr(:,:,j)=SPcondsum/SPcondnum;
%                 spstddrr(:,:,j)=std(SPcondtemp, 0, 3);
%     %         else
%     %             spstd(:,:,j)=0;
%             end
%             clear SPcondtemp;
...
%% SP for SPcond by Hisashi on 081003 and 090125
    if flagspcond==1
        for j=NStim+1:NStim+NSPcond
            condlist1=getfield(cell2struct(SPcond(j-NStim, 2), 'junk'), 'junk');
            condlist2=getfield(cell2struct(SPcond(j-NStim, 3), 'junk'), 'junk');
%             sizecondlist1=size(condlist1,2)
%             sizecondlist2=size(condlist2,2)
%             SPsubblockcounter=0;
            SPcondtemp=zeros(framenum,masknum,blockselectnum*size(condlist1,2));
            SPcondsum=zeros(framenum,masknum);
...
%                 spavg(:,:,j)=SPcondsum/SPsubblockcounter;
                ...
                if flagspmedian
                	spmeddrr(:,:,j)=median(SPcondtemp(:,:,1:SPcondsumnum), 3);
                end
            end
            clear SPcondtemp;
        end
