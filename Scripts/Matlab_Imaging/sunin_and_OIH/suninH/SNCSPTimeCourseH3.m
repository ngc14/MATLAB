...
% 
% Ver 3.0.1 (150224) sumtype = 3 was introduced. Subtraction will be done when at least one of trials is success in all blocks. Just for check
%                   Values in condlist2 were once averaged, then subtracted from valuse in condlist1. This option should not be used for statistical purpose. 
            ...
            if sumtype == 3 % 150224 HT
                ...
                for k=blockselect  % start block by block processing                	
                    if flagSPblanksubtraction == 1 %110301 HT
                        ...
                        end % for m=condlistBSub2
                        if SPcondcounterBSub2 > 0
                            spavgBsub2=SPcondtempBSub2/SPcondcounterBSub2;
                        end
                    end % if flagSPblanksubtraction == 1
				
                    for m=condlist2 %100419 HT                    
                        if  goodstim(k, m)==1 && ~(flagSPblanksubtraction == 1 && SPcondcounterBSub2 == 0)
            ...
            for k=blockselect  % start block by block processing
				...
				if sumtype==1 || sumtype==3
					if flagSPblanksubtraction == 0 %110301 HT
						...
						if SPcondcounter1>0 && (SPcondcounterBSub1>0 || sumtype==3)
							if SPcondcounter2>0 && (SPcondcounterBSub2>0 || sumtype==3)
								for n=1:SPcondcounter1
									SPcondtemp(:,:,SPcondsumnum+n)=SPcondtemp1(:,:,n)-SPcondtemp2/SPcondcounter2;
								end                            
							elseif size(condlist2,2)==0
                ...
                elseif sumtype==2
                    if flagSPblanksubtraction == 0 %110301 HT
						...
						if SPcondcounter1==size(condlist1,2) && (SPcondcounterBSub1 > 0 || sumtype==3)
							if SPcondcounter2==size(condlist2,2) && SPcondcounter2~=0 && (SPcondcounterBSub2 >0 || sumtype==3)
								for n=1:SPcondcounter1
									SPcondtemp(:,:,SPcondsumnum+n)=SPcondtemp1(:,:,n)-SPcondtemp2/SPcondcounter2; %need to be improved 100419 HT
								end                            
							elseif size(condlist2,2)==0
