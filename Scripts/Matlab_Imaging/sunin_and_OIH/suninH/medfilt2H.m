...
%   $Revision: 5.18.4.9 $  $Date: 2006/06/15 20:09:13 $
%
%
...
if (rem(prod(mn), 2) == 1)
    order = (prod(mn)+1)/2;
    b = ordfilt2H(a, order, domain, padopt);
else
    order1 = prod(mn)/2;
    order2 = order1+1;
    b = ordfilt2H(a, order1, domain, padopt);
    b2 = ordfilt2H(a, order2, domain, padopt);
	if islogical(b)
		b = b | b2;
	else
...
elseif isempty(charLocation)
    ...
    options = {'indexed', 'zeros', 'symmetric', 'circular', 'replicate'};
