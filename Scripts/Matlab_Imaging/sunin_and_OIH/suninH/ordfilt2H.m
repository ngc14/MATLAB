...
%   $Revision: 5.17.4.8 $  $Date: 2006/06/15 20:09:15 $
%
%   091011 Added 'circular' and 'replicate' for PADOPT. The default is 'symmetric'.; Hisashi
...
elseif (strcmp(padopt, 'circular'))
    A = padarray(A, padSize * [1 1], 'circular', 'both');
...
iptchecknargin(3,5,nargin,mfilename);

A = varargin{1};
order = varargin{2};
domain = varargin{3};
options = {'zeros', 'ones', 'symmetric', 'circular', 'replicate'};
% padopt of 'ones' is for supporting medfilt2; it is undocumented.

if (nargin == 4)
  if (ischar(varargin{4}))
    padopt = iptcheckstrs(varargin{4},options,mfilename,'PADOPT',4);
  else
    s = varargin{4};
  end
...
elseif (nargin == 5)
  s = varargin{4};
  padopt = iptcheckstrs(varargin{5},options,mfilename,'PADOPT',5);  
end

% make sure that arguments are valid
iptcheckinput(order,{'double'},{'real','scalar','integer'},mfilename, ...
              'ORDER',2);
...
if ~isempty(s)
  ...
  iptcheckinput(A, {'double'}, {'2d','real'}, mfilename, 'A', 1);
  s = s(find(domain));
  iptcheckinput(s, {'double'}, {'real'}, mfilename, 'S', 4);
else
