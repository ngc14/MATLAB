...
%%      (080130) H2.0 you can choose precision type (precisiontype='single' or 'double') 
...
if flagmasking>0 
    ...
    case 2     % masks are from coordinates, create masks in memory here
        if isempty(maskname)
            ...
            maskname=char(masknametemp(:,1:end-6));
        else
            masknum=size(maskname,1);
        end
...
%         end                
        for i=1:masknum
            domainfidx=fopen (strcat(resultdriver, 'expresult\', expname, 'masks\', maskname(i,:), '_x.txt'), 'r'); %Hisashi
            domainfidy=fopen (strcat(resultdriver, 'expresult\', expname, 'masks\', maskname(i,:), '_y.txt'), 'r'); %Hisashi
        ...
        if (flagsp>0)
            for j=1:masknum
                for f=1:FramesPerStim
                    switch precisiontype
                        case 'single'
                            maskmap= single(DCFrames(:,:,f)).*double(mask(:, :, j));
...
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%HS Save processed block files
% if (flagsaveprcblk)    % save processed block files 
%     for i=1:NStim
%         prcblock(:,:,1,i)=singlemap(:,:,i);
%     end
...
%     if ~isdir([blockfolder, 'prcblk\'])
%         mkdir(blockfolder, 'prcblk\');    
%     end
%     filenameprc=strcat(getfield(cell2struct(filename(k), 'junk'), 'junk'));
%     fidnewblock=fopen(strcat(blockfolder, 'prcblk/', filenameprc(1:end-4), '.blp'), 'wb');
%     fwrite(fidnewblock, header, 'uint8');
%     prcblock=permute(prcblock, [2,1,3,4]);
%     fwrite(fidnewblock, prcblock, 'float32');
%     fclose(fidnewblock);    
% end
% 
    ...
    for j=1:size(Smap, 1)						% note: not differentiate goodstim=1 or 0, get single block map anyway
        ...
        if flagtrialfilter & (flaghpfilter|flaglpfilter)
	        singlemap(:,:,i+j)=OIEasyFilterH(Sum1-Sum2, lpfmethod, lpkernelsize, hpfmethod, hpkernelsize); %modified by Hisashi
	    else
...
%     for (j=1:masknum) %added by Hisashi    
    ...
    for (i=1:NStim)
        ...
        fprintf(superfid2, '\r');
	    for (j=1:masknum)
	        fprintf(superfid2, '%s\t', maskname(j, :));
            for (f=1:FramesPerStim)
