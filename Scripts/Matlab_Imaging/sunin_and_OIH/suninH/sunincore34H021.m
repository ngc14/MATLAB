...
%%      (080228) H2.1 You can averaged block file for analysis which you saved
...
resultfolder=[resultdriver, 'expresult\', expname, runname, resultname];
...
if ~isdir([resultdriver, 'expresult\', expname, runname])
    mkdir([resultdriver, 'expresult\', expname], runname);    
end
if ~isdir([resultdriver, 'expresult\', expname, runname, resultname])
    mkdir([resultdriver, 'expresult\', expname, runname], resultname);    
end
...
clipmask = strcat(resultdriver, 'expresult\', expname, 'masks\clipmask\default.bmp');     % can use a blood vessel map for clip masking, only for clipmethod==2
filtermask = strcat(resultdriver, 'expresult\', expname, 'masks\filtermask\default.bmp');     % can use a blood vessel map for clip masking, only for clipmethod==2
...
if flagspmask>0 
    maskfolder=strcat(resultdriver, 'expresult\', expname, 'masks\spmask\');
    switch flagspmask
...
%         end                
        for i=1:masknum
            domainfidx=fopen (strcat(resultdriver, 'expresult\', expname, 'masks\spmask\', spmaskname(i,:), '_x.txt'), 'r'); %Hisashi
            domainfidy=fopen (strcat(resultdriver, 'expresult\', expname, 'masks\spmask\', spmaskname(i,:), '_y.txt'), 'r'); %Hisashi
            ...
            if (savemaskmap==1)     % output memory masks for evaluating if selected 
                maskmap=mask(:,:,i)*255;
                imwrite(uint8(maskmap), strcat(resultdriver, 'expresult\', expname, 'masks', spmaskname(i,:), '_1.bmp'), 'bmp');
            end
        end
        clear maskmap;
    end
