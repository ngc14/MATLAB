...
%% test: subtract dark frame from dark line
    ...
    %            FrameHeight=FrameHeight-2*cropsize;
        ...
        if (flagalign==41 & datasource~=3) % Modified by Hisashi on 080229
            ...
            if (flagsaveaverageblk | (flagsp>0))
                for j = 1:FramesPerStim
                    DCFrames(:,:,j) = OIShift(DCFrames(:,:,j), sfx1((k-1)*NStim+i), sfy1((k-1)*NStim+i)); % by Hisashi on 080229
                end
...
%         if (flagalign==41 & (flagsaveaverageblk | (flagsp>0)) & datasource~=3)
%             for j = 1:FramesPerStim
%                 DCFrames(:,:,j) = OIShift(DCFrames(:,:,j), sfx1((k-1)*NStim+i), sfy1((k-1)*NStim+i)); % by Hisashi on 080229
%             end
%             avgblock(:,:,:,currentstimid)=avgblock(:,:,:,currentstimid)+DCFrames;       % Note, simple summaton, not dRR, need compare with sum of dRR
%         end
        ...
        if (flagsp>0)
            if flagtrialfilter && (flaghpfilter||flaglpfilter) % by Hisashi on 080229
...
%     for (j=1:masknum) %added by Hisashi    
	...
	% superpix2 is just another format for sp1 (same stim are grouped together) %added by Hisashi 
...
if flaglpfilter | flaghpfilter>0
    if flagtrialfilter == 0
        SNCFilterSMapH; %by Hisashi
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Save sum maps
if flagmap>0
    SNCSaveSMapH; % by Hisashi
end
