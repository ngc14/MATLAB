...
%%      (080303) H2.3 In superpixel analysis, image is filtered and shifted correctly.
...
if flagvector==2
	...
	Polarmap=OIPolarH(singlemap, lut, vectmask, vectclipsd, lowpass, highpass, LPfiltermethod, HPfiltermethod); %modified by Hisashi 071022
...
filtermaskname = strcat(resultdriver, 'expt1\', expname, 'masks\filtermask\', filtermaskname);     % can use a blood vessel map for clip masking, only for clipmethod==2
...
for k=blockselect  % start block by block processing
    ...
    %            FrameHeight=FrameHeight-2*cropsize;
        ...
        if flagtrialfilter && (flaghpfilter||flaglpfilter) && datasource~=3 %added by Hisashi on 080229
            SumFrame(:,:,currentstimid)=OIEasyFilterH(SumFrame(:,:,currentstimid), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermaskname); %modified by Hisashi		
        end  
...
%                 fprintf ('\r\rflagsp&flaghpfilter1');
                 for f=1:FramesPerStim
                    DCFrames2(:,:,f)=OIEasyFilterH(DCFrames(:,:,f), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermaskname); %modified by Hisashi  
                 end
            else
                DCFrames2=DCFrames;
            end
        ...
        if (flagsp>0)
            if (flaghpfilter||flaglpfilter) && flagsptrialfilter && datasource~=3 %added by Hisashi on 080307
            	% Note :: For superpixel analysis, image is filtered trial-by-trial (080307 Hisashi)
%                 fprintf ('\r\rflagsp&flaghpfilter2');
	            if ~(flagsaveaverageblk && flagtrialfilter)
                    for f=1:FramesPerStim
                        DCFrames3(:,:,f)=OIEasyFilterH(DCFrames(:,:,f), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermaskname); %modified by Hisashi  
                    end
                else
                    DCFrames3=DCFrames2;
                end
...
if (flagsaveaverageblk && datasource~=3)    % save averaged block file 
    for i=1:NStim
	    ...
	    if (flaghpfilter||flaglpfilter) && ~flagtrialfilter  %added by Hisashi on 080307
	    	for f=1:FramesPerStim
	        	avgblock(:,:,f,i)=OIEasyFilterH(avgblock(:,:,f,i), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermaskname); %modified by Hisashi  
	        end
        end
    end 
...
if (flagsp>0)
    if flagsavespvalues==1
        save strcat(blockfolder, 'spvalues.mat') spvalues;
        save strcat(blockfolder, 'stimsumnum.mat') stimsumnum;
    end
    if flagloadspvalues==1
        load strcat(blockfolder, 'spvalues.mat');
        load strcat(blockfolder, 'stimsumnum.mat');
    end
