...
%%      The types of 'flagmap' are defined by '1', '2', '4', and '8'. You can choose several types by summing these numbers
...
if flagmap==4 || flagmap==8 || flagmap==12
...
smapnum = size(Smap, 1);       % Subtraction map number
mapnum = NStim + smapnum;   % total map number
if flagsp > 0 % by Hisashi on 080308
	SPcondnum = size(SPcond, 1);
end
...
if (flaggoodstim==1 & datasource~=3) % load good stim condition file
	fidgoodstim=fopen(strcat(blockfolder, 'goodstim.txt'), 'r'); %Hisashi 071023
    ...
    if flagmap == 4 || flagmap == 8 || flagmap == 12   % user reqires single block maps?
        fprintf('Warning: ''goodstim=1'' and ''flagmap=4 or 8'', you may not get all single block maps, anykey to continue...');
        pause;
    end
else
...
if (flagalign==41 & datasource~=3)  % Load shift parameters generated by flagalign==4 process (include '.1' file for all first frame shift and '.3' file for all rest frames)
    [sfname, sfblock, sfstim, sfframe, sfx1, sfy1, sfcoor]=textread(strcat(blockfolder, 'shiftinput.2.txt'), '%s %d %d %d %f %f %f'); %Hisashi 071023
...
switch precisiontype %by Hisashi
    case 'single'
        ...
        if (flagsaveaverageblk & datasource~=3)
            avgblock=zeros(FrameHeight, FrameWidth, FramesPerStim, NStim, 'single');   % note: memory waste
        end
    case 'double'
        if (flagspmask>0) 
...
for k=blockselect  % start block by block processing
    ...
    %            FrameHeight=FrameHeight-2*cropsize;
        ...
        if flagtrialfilter && (flaghpfilter||flaglpfilter) %added by Hisashi on 080229
            SumFrame(:,:,currentstimid)=OIEasyFilterH(SumFrame(:,:,currentstimid), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermask); %modified by Hisashi		
        end  
        ...
        if flagsaveaverageblk && datasource~=3 %added by Hisashi on 080307
        	if flagtrialfilter & (flaghpfilter||flaglpfilter)
                 for f=1:FramesPerStim
                    DCFrames(:,:,f)=OIEasyFilterH(DCFrames(:,:,f), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermask); %modified by Hisashi  
                end
            end       	
        ...
        if (flagsp>0)
            if (flaghpfilter||flaglpfilter) && ~(flagsaveaverageblk && flagtrialfilter) && datasource~=3 %added by Hisashi on 080307
	        ...
	        if (flagalign==41 && datasource~=3) %added by Hisashi on 080307
	            for f = 1:FramesPerStim
                    DCFrames(:,:,f) = OIShift(DCFrames(:,:,f), sfx1((k-1)*NStim+i), sfy1((k-1)*NStim+i));
                end
            end
            for j=1:masknum
                for f=1:FramesPerStim
                    switch precisiontype
                        case 'single'
                            maskmap= single(DCFrames(:,:,f)).*single(mask(:, :, j));
    ...
    if (flagmap~=0)
        for j=1:size(Smap, 1)						% note: not differentiate goodstim=1 or 0, get single block map anyway
        	...
        	if ~flagtrialfilter && (flaghpfilter||flaglpfilter) %added by Hisashi on 080229
            	singlemap(:,:,NStim+j)=OIEasyFilterH(singlemap(:,:,NStim+j), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermask); %modified by Hisashi		
        	end     
        end
    ...
    %	end            
        ...
        if flagmap==4  || flagmap==5 || flagmap==6 || flagmap==12
        ...
        if flagmap==8 || flagmap==9 || flagmap==10 || flagmap==12
            for i=1:mapnum
                mapname=getfield(cell2struct(mapnames(i), 'junk'), 'junk');
                [singlemap(:,:,i), mapinfo(1, k, i), mapinfo(2, k, i), mapinfo(3, k, i)]=OIClipH(singlemap(:,:,i), clipmethod, clipvalue, immask);       % mapinfo will be write into 'mapinfo.txt'
                map=norm_to_uint8(singlemap(:,:,i));             
...
if (flagsaveaverageblk && datasource~=3)    % save averaged block file 
    for i=1:NStim
	    ...
	    if (flaghpfilter||flaglpfilter) && ~flagtrialfilter  %added by Hisashi on 080307
	    	for f=1:FramesPerStim
	        	avgblock(:,:,f,i)=OIEasyFilterH(avgblock(:,:,f,i), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermask); %modified by Hisashi  
	        end
	    end
        if flagalign==41 %added by Hisashi on 080307
            for f=1:FramesPerStim
            	avgblock(:,:,f,i) = OIShift(avgblock(:,:,f,i), sfx1((k-1)*NStim+i), sfy1((k-1)*NStim+i));
        	end
    	end	    
    end 
...
if (flagsp>0)
	...
	spsem=zeros([FramesPerStim, masknum, NStim]);	%added by Hisashi on 080307
    for i=1:NStim  
		...
		spsem(:,:,i)=std(spvalues(:,:,i,1:stimsumnum(i)),0,4)/sqrt(stimsumnum(i));
    end
    spavg=reshape(spavg, [FramesPerStim, masknum, NStim]);
    spstd=reshape(spstd, [FramesPerStim, masknum, NStim]);
    spsem=reshape(spsem, [FramesPerStim, masknum, NStim]);
    
    for i=1:size(SPcond, 1)
    	condlist1=getfield(cell2struct(SPcond(i, 2), 'junk'), 'junk');
    	for n=condlist1
    		for m=1:stimsumnum(n)
    			j=j+1;
        		SPcondvalue1(j)=spvalues(:,:,n,m);
    	end    
...
for i=startmap:mapnum
    ...
    [maptemp, framemedian, lowClip, highClip] = OIClipH(maps(:,:,i), clipmethod, clipvalue, immask);
...
%%%%%%%%%%%
    ...
    superfid1= fopen(strcat(resultfolder, strcat('superpixel_original1', '.txt')), 'w');  % for sp avg & stdev output, 1, 2 are two different formats
    superfid2= fopen(strcat(resultfolder, strcat('superpixel_original2', '.txt')), 'w');    
    ...
    for (j=1:masknum)
        fprintf(superfid1, '%s\t', spmaskname(j, :));
        if j==1
    ...
    for (j=1:masknum)
        fprintf(superfid1, '%s\r', spmaskname(j, :));
        for (i=1:NStim)
            fprintf(superfid1, 'stim%d\t', i);
            for (f=1:FramesPerStim)
    ...
    fprintf(superfid2, 'original value\r');
    fprintf(superfid2, 'f%d\t', 1:FramesPerStim);
    fprintf(superfid2, '\r');
    for (i=1:NStim)
        fprintf(superfid2, 'stim%d\t', i);
        if i==1
            fprintf(superfid2, '%f\t', xlabel);
        end
        fprintf(superfid2, '\r');
	    for (j=1:masknum)
	        fprintf(superfid2, '%s\t', spmaskname(j, :));
            for (f=1:FramesPerStim)
                fprintf(superfid2, '%6.5e\t', spavg(f, j, i));			% original value
            end
            fprintf(superfid2, '\r');
        end
    end
    fprintf(superfid2, '\r\r_______________SD_________\r\t');
    for (f=1:FramesPerStim)
        fprintf(superfid2, 'f%d\t', f);
    end
    fprintf(superfid2, '\r');
    for (i=1:NStim)
        fprintf(superfid2, 'stim%d\r', i);
	    for (j=1:masknum)
	        fprintf(superfid2, '%s\t', spmaskname(j, :));
            for (f=1:FramesPerStim)
                if blocknum>1
	                fprintf(superfid2, '%6.5e\t', spstd(f, j, i));
	            end
            end
            fprintf(superfid2, '\r');
        end
...
%     fclose(superfid1);
%     fclose(superfid2);
%     end %for j by Hisashi
    
    % dRR 	
    superfid1a= fopen(strcat(resultfolder, strcat('superpixeldRR1', '.txt')), 'w');  % for sp avg & stdev output, 1, 2 are two different formats
    superfid2a= fopen(strcat(resultfolder, strcat('superpixeldRR2', '.txt')), 'w');
...
%    			spvaluestemp(:, j, i, k)=(spvalues(:,j,i,k)-spvalues(1,j,i,k))/spvalues(1,j,i,k);
    			spvaluesfframetemp=mean(spvalues(fframe,j,i,k))); %modified by Hisashi on 080307
    			spvaluestemp(:, j, i, k)=(spvalues(:,j,i,k)-spvaluesfframetemp/spvaluesfframetemp;
    		end
    	end
    end
    for i=1:NStim  
    ...
    for (j=1:masknum)
        fprintf(superfid1a, '%s\t', spmaskname(j, :));
        if j==1
    ...
    fprintf(superfid1a, '\r');
    for (j=1:masknum)
        fprintf(superfid1a, '%s\r', spmaskname(j, :));
        for (i=1:NStim)
            fprintf(superfid1a, 'stim%d\t', i);
            for (f=1:FramesPerStim)
                if blocknum>1
    ...
    for (i=1:NStim)
        ...
        fprintf(superfid2a, '\r');
	    for (j=1:masknum)
	        fprintf(superfid2a, '%s\t', spmaskname(j, :));
            for (f=1:FramesPerStim)
    ...
    for (j=1:masknum)
        fprintf(superfid3a, 'mask: %s\t', spmaskname(j, :));
        for (i=1:NStim)
        	fprintf(superfid3a, '\t');
        end
	end
