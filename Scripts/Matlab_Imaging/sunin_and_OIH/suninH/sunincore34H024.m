...
if flagvector==2
	...
	Polarmap=OIPolarH2(singlemap, lut, vectmask, vectclipsd, lowpass, highpass, LPfiltermethod, HPfiltermethod, flagmaskfilter); %modified by Hisashi 071022
...
if (flagloadspvalues~=1)
    for k=blockselect  % start block by block processing
        if blocknum>1
            if sum(stimsumnum)>0 && flagfirst==0 % && k==blockselect(2)    % just for estimate process time 
                ...
                fprintf('\rTime for all (%d) blocks: %5.2f hours\r', blocknum, blockproctime/sum(stimsumnum)*sum(sum(goodstim))/3600);
                flagfirst=1;
            end
        end
        ...
        %            FrameHeight=FrameHeight-2*cropsize;
            ...
            if flagtrialfilter && (flaghpfilter||flaglpfilter) && datasource~=3 %added by Hisashi on 080229
                SumFrame(:,:,currentstimid)=OIEasyFilterH(SumFrame(:,:,currentstimid), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter); %modified by Hisashi		
            end  
    ...
    %                 fprintf ('\r\rflagsp&flaghpfilter1');
                     for f=1:FramesPerStim
                        DCFrames2(:,:,f)=OIEasyFilterH(DCFrames(:,:,f), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter); %modified by Hisashi  
                     end
                else
                    DCFrames2=DCFrames;
                end
    ...
    %                 fprintf ('\r\rflagsp&flaghpfilter2');
                    if ~(flagsaveaverageblk && flagtrialfilter)
                        for f=1:FramesPerStim
                            DCFrames3(:,:,f)=OIEasyFilterH(DCFrames(:,:,f), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter); %modified by Hisashi  
                        end
...
if (flagsaveaverageblk && datasource~=3)    % save averaged block file 
    for i=1:NStim
	    ...
	    if (flaghpfilter||flaglpfilter) && ~flagtrialfilter  %added by Hisashi on 080307
	    	for f=1:FramesPerStim
	        	avgblock(:,:,f,i)=OIEasyFilterH(avgblock(:,:,f,i), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter); %modified by Hisashi  
	        end
        end
    end 
...
if flagmap>0
    if flaglpfilter | flaghpfilter>0
        if flagtrialfilter == 0
            SNCFilterSMapH; % modified by Hisashi
        end
    end
