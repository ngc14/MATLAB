...
if flagspmask>0 
    ...
    case 1      % for map-loading, Mask bmp files are 0-1 monochrome bmp files, desired areas are 1's
        ...
        for i=1:masknum
            spmaskname(i,:);
            mask(:,:,i) = imread (strcat(maskfolder, spmaskname(i,:)), 'bmp');
        end
...
%fprintf(fidmasterlog, '%s\t%s\t%d\t%d\t%d\t%f\t%f\t%f\t%f\t%f\t%d\t%d\t%d\t%d\t%d\t%d\tstart%d-%d-%d %d:%d.%d\r',ext, blockfolder, blocknum, operation, flagalign, shiftrange(1), shiftrange(2), shiftrange(3), shiftrange(4), shiftstep1, clipmethod, clipvalue, sframe, fframe, bframe, flagspmask, fix(clock));
...
switch precisiontype %by Hisashi
    case 'single'
        if (flagspmask>0) 
            domainvalues=zeros(masknum, mapnum, blocknum, 'single');    %store average value for each mask, each stim, and each block, can expand to include analysis map values
        end
        ...
        if flagsp==1
            spvalues=NaN(FramesPerStim+2,masknum,NStim,blocknum, 'single');
        end
    ...
    case 'double'
        ...
        stimsumnum=zeros(NStim,1);	%keep how many stim was summed. (in randomized & goodstim cases, it may not same for each condition).
        if flagsp==1
            spvalues=NaN(FramesPerStim+2,masknum,NStim,blocknum);
        end
        if (flagsaveaverageblk & datasource~=3)
    ...
    for k=blockselect  % start block by block processing
        if blocknum>1
            if sum(stimsumnum)>0 && flagfirst==0 % && k==blockselect(2)    % just for estimate process time 
                ...
                fprintf('\rTime for all (%d) blocks: %5.2f hours\r', blocknum, blockproctime/sum(stimsumnum)*sum(sum(goodstim))*blocknum/blockfilenum/3600);
                flagfirst=1;
            end
        end
