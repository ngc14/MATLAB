...
%%      (080422) H2.7 'flagsavespdataall' was introduced for save trial-by-trial raw data in a text file     
%%                          'flagsaveframeave' was introduced
%%      (080422) H2.5 'flagvectormaskfilter' was introduced.
%%                          SNCFilterSMapH2.m was introduced.
%%      (080303) H2.3 Superpixel analysis part was updated and t-test function was introduced 
%%                          
%%      (080303) H2.2 The default input and output folder name, 'Expt' and
%%      'expt1' was changed to 'expt0' and 'expt1', respectively.
%%      (080228) H2.1 You can averaged block file for analysis which you
%%      saved
%%      previously by 'flagsaveaverageblk=1'. The extention of this type of file
%%      is '.aBLK'. Set 'datasource=3'.
%%      (080130) H2.0 You can choose precision type (precisiontype='single'
%%      or 'double') . Using 'single' precision may reduce memory usage.
%%      (080115) H1.1 Add mask filtering function in SNCFilterSMapH.m (flagmaskfilter)
%%      (060502) H1.0 Add trial by trial filtering (flagtrialfilter)
...
resultfolder=[resultdriver, 'expt1\', expname, runname, resultname];
runfolder=[resultdriver, 'expt1\', expname, runname];
blockfolder = strcat(datadriver, datafolder, expname, runname); 
if ~isdir([resultdriver, 'expt1\'])
    mkdir(resultdriver, 'expt1\');    
end
if ~isdir([resultdriver, 'expt1\', expname])
    mkdir([resultdriver, 'expt1\'], expname);    
end
if ~isdir([resultdriver, 'expt1\', expname, runname])
    mkdir([resultdriver, 'expt1\', expname], runname);    
end
if ~isdir([resultdriver, 'expt1\', expname, runname, resultname])
    mkdir([resultdriver, 'expt1\', expname, runname], resultname);    
end
...
if flagvector==2
	...
	Polarmap=OIPolarH2(singlemap, lut, vectmask, vectclipsd, lowpass, highpass, LPfiltermethod, HPfiltermethod, flagvectormaskfilter, filtermask); %modified by Hisashi 071022
...
clipmask = strcat(resultdriver, 'expt1\', expname, 'masks\clipmask\default.bmp');     % can use a blood vessel map for clip masking, only for clipmethod==2
...
if flagspmask>0 
    maskfolder=strcat(resultdriver, 'expt1\', expname, 'masks\spmask\');
    switch flagspmask
    case 1      % for map-loading, Mask bmp files are 0-1 monochrome bmp files, desired areas are 1's
        ...
        for i=1:masknum
            ...
            mask(:,:,i) = imread (strcat(maskfolder, spmaskname(i,:), '.bmp'), 'bmp');
        end
...
%         end                
        for i=1:masknum
            domainfidx=fopen (strcat(resultdriver, 'expt1\', expname, 'masks\spmask\', spmaskname(i,:), '_x.txt'), 'r'); %Hisashi
            domainfidy=fopen (strcat(resultdriver, 'expt1\', expname, 'masks\spmask\', spmaskname(i,:), '_y.txt'), 'r'); %Hisashi
            ...
            if (savemaskmap==1)     % output memory masks for evaluating if selected 
                maskmap=mask(:,:,i)*255;
                imwrite(uint8(maskmap), strcat(resultdriver, 'expt1\', expname, 'masks\spmask_', spmaskname(i,:), ext, '.bmp'), 'bmp');
            end
        end
        clear maskmap;
    end
maskfilename
fprintf('\nFound %d masks.\n', masknum);
mapinfo=zeros(3, blockselectnum, mapnum);      %save (median, lowclip, highclip) for each maps, blknum, mapnum 
end
...
if (flaggoodstim==1 & datasource~=3) % load good stim condition file
	fidgoodstim=fopen(strcat(runfolder, 'goodstim.txt'), 'r'); %Hisashi 071023
...
%    shiftmask = imread(strcat(resultdriver, 'expt1\', expname, 'masks\shiftmask1.bmp'), 'bmp');     % bv mask for shifting method3
end
...
if (flaghpfilter||flaglpfilter)
    ...
    if flaglpfilter
        fprintf('\rImages will be low-pass filterred with \''%s\'' of %g.', lpfmethod, lpkernelsize);
    end
    if flaghpfilter
        fprintf('\rImages will be high-pass filterred with \''%s\'' of %g.', hpfmethod, hpkernelsize);
    end
    if flagmaskfilter
    ...
    if (flagtrialfilter && flagsaveaverageblk)
        fprintf('\rSpatial filtering will be applied frame-by-frame for making an averaged block file (aBLK).');
    end
    if (flagsp && flagsptrialfilter)
...
fprintf('\rResults will be saved in   "%s"\n', resultfolder);
...
switch precisiontype %by Hisashi
    case 'single'
        ...
        if (flagsaveaverageblk & datasource~=3)
            avgblock=zeros(FrameHeight, FrameWidth, FramesPerStim, NStim, 'single');
        end
    case 'double'
        if (flagspmask>0) 
    ...
    for k=blockselect  % start block by block processing
        if blockselectnum>1
            if sum(stimsumnum)>0 && flagfirst==0 % && k==blockselect(2)    % just for estimate process time 
                ...
                temptime=blockproctime/sum(stimsumnum)*sum(sum(goodstim))*(blockselectnum/blockfilenum);
        ...
        %            FrameHeight=FrameHeight-2*cropsize;
            ...
            if flagtrialfilter && (flaghpfilter||flaglpfilter) && datasource~=3 %added by Hisashi on 080229
                SumFrame(:,:,currentstimid)=OIEasyFilterH2(SumFrame(:,:,currentstimid), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermask); %modified by Hisashi		
            end  
            ...
            if flagsaveaverageblk && datasource~=3 %added by Hisashi on 080307
                if flagtrialfilter & (flaghpfilter||flaglpfilter)
    %                 fprintf ('\r\rflagsp&flaghpfilter1');
                     for f=1:FramesPerStim
                        DCFrames2(:,:,f)=OIEasyFilterH2(DCFrames(:,:,f), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermask); %modified by Hisashi  
                     end
                else
                    DCFrames2=DCFrames;
                end
                if flagalign==41 %added by Hisashi on 080307
                    for f=1:FramesPerStim
                        DCFrames2(:,:,f) = OIShift(DCFrames2(:,:,f), sfx1((k-1)*NStim+i), sfy1((k-1)*NStim+i));
                    end
                end	    
                avgblock(:,:,:,currentstimid)=avgblock(:,:,:,currentstimid)+DCFrames2;       % Note, simple summaton, not dRR, need compare with sum of dRR
                if flagsp==0 || (flagsp>0 && ~flagsptrialfilter && ~flagalign>0 )
                    clear DCFrames2;
                end
            end
            ...
            if (flagsp>0)
                if (flaghpfilter||flaglpfilter) && flagsptrialfilter && datasource~=3 %added by Hisashi on 080307
                    % Note :: For superpixel analysis, filtering is applied trial-by-trial (080307 Hisashi)
    %                 fprintf ('\r\rflagsp&flaghpfilter2');
                    if ~(flagsaveaverageblk && flagtrialfilter)
                        for f=1:FramesPerStim
                            DCFrames3(:,:,f)=OIEasyFilterH2(DCFrames(:,:,f), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermask); %modified by Hisashi  
                        end
%                         fprintf('flagsptrialfilter');
                    else
                        DCFrames3=DCFrames2;
                    end
                else
                ...
                for j=1:masknum
                    for f=1:framenum
                        if f<=FramesPerStim
                            switch precisiontype
                                case 'single'
                                    maskmap= single(DCFrames3(:,:,f)).*single(mask(:, :, j));
                                    spvalues(f,j,currentstimid,stimsumnum(currentstimid))=sum(sum(maskmap))/sum(sum(single(mask(:,:,j))));
                                case 'double'
                                    maskmap= double(DCFrames3(:,:,f)).*double(mask(:, :, j));
    ...
    %         else
            ...
            if flagmap==3
                for i=NStim+1:mapnum
                    ...
                    [singlemap(:,:,i), mapinfo(1, k, i), mapinfo(2, k, i), mapinfo(3, k, i)]=OIClipH(singlemap(:,:,i), clipmethod, clipvalue, immask);       % mapinfo will be write into 'mapinfo.txt'
            ...
            if (flagquantify~=0)
                for i=1:mapnum %NStim       % currently only output single-condition values, change to 'mapnum' here to include analysis maps
                    for j=1:masknum
                        switch precisiontype
                            case 'single'
                                maskmap= singlemap(:,:,i).*single(mask(:, :, j));
                                domainvalues(j,i,k)=sum(sum(maskmap))/sum(sum(single(mask(:,:,j)))); % avg value for block 'k', stim 'i', and mask 'j'.
                            case 'double'
        ...
        if (mod(k, tempsaveblocks)==0)  % set tempsaveblocks=0 or 999 to avoid save tempmaps
            for i=1:mapnum    
               ...
               [tempmaps(:,:,i), framemedian, lowClip, highClip]=OIClipH(tempmaps(:,:,i), clipmethod, clipvalue, immask);
        ...
        if (saveaccum)  % save the temporary accumulate maps
            for i=accummapnum+NStim    % use 'mapnum' to save all maps (a lot)
                mapname=getfield(cell2struct(mapnames(i), 'junk'), 'junk');
                [tempaccu, framemedian, lowClip, highClip]=OIClipH(maps(:,:,i), clipmethod, clipvalue, immask);
...
% average 
if (flagsaveaverageblk && datasource~=3)    % save averaged block file 
    for i=1:NStim
        avgblock(:,:,:,i)=avgblock(:,:,:,i)/stimsumnum(i);
% 	    if (flaghpfilter||flaglpfilter) && ~flagtrialfilter  %added by Hisashi on 080307
% 	    	for f=1:FramesPerStim
% 	        	avgblock(:,:,f,i)=OIEasyFilterH2(avgblock(:,:,f,i), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize, flagmaskfilter, filtermask); %modified by Hisashi  
% 	        end
%         end
    end 
    ...
    fidnewblock=fopen(strcat(blockfolder, ablkname, '_avg', '.aBLK'), 'wb');
...
%     spvalues=reshape(spvalues, FramesPerStim+2,masknum,NStim,max(stimsumnum));     % don't know why this is necessory
    ...
    spavg=zeros(framenum, masknum, NStim);
	spstd=zeros([framenum, masknum, NStim]);	
    for i=1:NStim  
    ...
    % SP for SPcond by Hisashi on 080316
    if flagspcond==1
        for j=NStim+1:NStim+NSPcond
            ...
            SPcondnum=0;
            for i=condlist1
                SPcondsum=SPcondsum+squeeze(sum(spvalues(:,:,i,1:stimsumnum(i)),4));
                SPcondnum=SPcondnum+stimsumnum(i);
                SPcondtemp(:,:,n:n+stimsumnum(i)-1)=squeeze(spvalues(:,:,i,1:stimsumnum(i)));
                n=n+stimsumnum(i);
            end
%             n
%             j-NStim
            spavg(:,:,j)=SPcondsum/SPcondnum;
            if n>1
               spstd(:,:,j)=std(SPcondtemp, 0, 3);
            else
                spstd(:,:,j)=0;
            end
            stimsumnum(j)=SPcondnum;
            clear SPcondtemp;
        end
    end    
    ...
    superfida1= fopen(strcat(resultfolder, strcat('superpixel_raw1', '.txt')), 'w');  % for sp avg & stdev output, 1, 2 are two different formats
    superfida2= fopen(strcat(resultfolder, strcat('superpixel_raw2', '.txt')), 'w');    

    % superpix_original1
    fprintf(superfida1, 'original value\t');
    fprintf(superfida1, '%f\t', xlabel);
    fprintf(superfida1, '\r');
    for (j=1:masknum)
        fprintf(superfida1, '%s\t', spmaskname(j, :));
        fprintf(superfida1, 'f%d\t', 1:FramesPerStim);
        if flagsaveframeave
            fprintf(superfida1, 'fframe\t');
            fprintf(superfida1, 'sumrange\t');
        end
        fprintf(superfida1, '\r');
        for (i=1:NStim+NSPcond)
            if i<=NStim
                fprintf(superfida1, 'stim%d\t', i);
            else
                fprintf(superfida1, '%s\t', getfield(cell2struct(SPcond(i-NStim, 1), 'junk'), 'junk'));
            end
            for (f=1:framenum)
                fprintf(superfida1, '%6.5f\t', spavg(f, j, i));			% original value
            end
            fprintf(superfida1, '\r');
        end
    end
    fprintf(superfida1, '\r\r_____SD_____\r\t');
    fprintf(superfida1, '%f\t', xlabel);
    fprintf(superfida1, '\r');
    for (j=1:masknum)
        fprintf(superfida1, '%s\t', spmaskname(j, :));
        ...
        for (i=1:NStim+NSPcond)
            ...
            for (f=1:framenum)
                fprintf(superfida1, '%6.5f\t', spstd(f, j, i));
            end
            fprintf(superfida1, '\r');
        end
    end
    fprintf(superfida1, '\r\r_____Num_____\r\t');
    fprintf(superfida1, '%f\t', xlabel);
    fprintf(superfida1, '\r');
    for (j=1:masknum)
        fprintf(superfida1, '%s\t', spmaskname(j, :));
        ...
        for (i=1:NStim+NSPcond)
            ...
            for (f=1:framenum)
                fprintf(superfida1, '%d\t', stimsumnum(i));
            end
            fprintf(superfida1, '\r');
        end
    end
    fprintf(superfida1, '\r\r_____SEM_____\r\t');
    fprintf(superfida1, '%f\t', xlabel);
    fprintf(superfida1, '\r');
    for (j=1:masknum)
        fprintf(superfida1, '%s\t', spmaskname(j, :));
        ...
        for (i=1:NStim+NSPcond)
            ...
            for (f=1:framenum)
                fprintf(superfida1, '%6.5f\t', spstd(f, j, i)/sqrt(stimsumnum(i)));
            end
            fprintf(superfida1, '\r');
        end
    end
    ...
    fprintf(superfida2, 'original value\t');
    fprintf(superfida2, '%f\t', xlabel);
    fprintf(superfida2, '\r');
    for (i=1:NStim+NSPcond)
        if i<=NStim
            fprintf(superfida2, 'stim%d\t', i);
        else
            fprintf(superfida2, '%s\t', getfield(cell2struct(SPcond(i-NStim, 1), 'junk'), 'junk'));
        end
        fprintf(superfida2, 'f%d\t', 1:FramesPerStim);
        if flagsaveframeave
            fprintf(superfida1, 'fframe\t');
            fprintf(superfida1, 'sumrange\t');
        end
        fprintf(superfida2, '\r');
	    for (j=1:masknum)
	        fprintf(superfida2, '%s\t', spmaskname(j, :));
            for (f=1:framenum)
                fprintf(superfida2, '%6.5f\t', spavg(f, j, i));			% original value
            end
            fprintf(superfida2, '\r');
        end
    end
    fprintf(superfida2, '\r\r_____SD_____\r\t');
    fprintf(superfida2, '%f\t', xlabel);
    fprintf(superfida2, '\r');
    for (i=1:NStim+NSPcond)
        if i<=NStim
	    ...
	    for (j=1:masknum)
            ...
            for (f=1:framenum)
                if blockselectnum>1
	                fprintf(superfida2, '%6.5f\t', spstd(f, j, i));
	            end
            end
            fprintf(superfida2, '\r');
        end
    end
    fprintf(superfida2, '\r\r_____Num_____\r\t');
    fprintf(superfida2, '%f\t', xlabel);
    fprintf(superfida2, '\r');
    for (i=1:NStim+NSPcond)
        if i<=NStim
        ...
        fprintf(superfida2, '\r');
	    for (j=1:masknum)
	        fprintf(superfida2, '%d', spmaskname(j, :));
            for (f=1:framenum)
                if blockselectnum>1
	                fprintf(superfida2, '%6.5f\t', stimsumnum(i));
	            end
            end
            fprintf(superfida2, '\r');
        end
    end
    fprintf(superfida2, '\r\r_____SEM_____\r\t');
    fprintf(superfida2, '%f\t', xlabel);
    fprintf(superfida2, '\r');
    for (i=1:NStim+NSPcond)
        if i<=NStim
        ...
        for (j=1:masknum)
            ...
            for (f=1:framenum)
                if blockselectnum>1
	                fprintf(superfida2, '%6.5f\t', spstd(f, j, i)/sqrt(stimsumnum(i)));
	            end
            end
            fprintf(superfida2, '\r');
        end
...
%%% dRR 	
    superfidb1= fopen(strcat(resultfolder, strcat('superpixel_dRR1', '.txt')), 'w');  % for sp avg & stdev output, 1, 2 are two different formats
    superfidb2= fopen(strcat(resultfolder, strcat('superpixel_dRR2', '.txt')), 'w');
%     superfid3a= fopen(strcat(resultfolder, strcat('superpixeldRR3', '.txt')), 'w');  
    spvalues2=spvalues;
    spavg2=zeros(framenum, masknum, NStim);
	spstd2=zeros(framenum, masknum, NStim);	
   	for i=1:NStim
	    for k=1:stimsumnum(i)
    		for j=1:masknum
    			spvalues2(:, j, i, k)=(spvalues(:,j,i,k)-spvalues(1,j,i,k))/spvalues(1,j,i,k);
    		end
    	end
    end
    for i=1:NStim
        spavg2(:,:,i)=sum(spvalues2(:,:,i,1:stimsumnum(i)),4)/stimsumnum(i);     % average over blocks, note: if use goodstim, then number of sum maybe different
    end
 	for i=1:NStim	
 		spstd2(:,:,i)=std(spvalues2(:,:,i,1:stimsumnum(i)),0,4);
    end
    ...
    if flagspcond==1
        for j=NStim+1:NStim+NSPcond
            ...
            SPcondnum=0;
            for i=condlist1
                SPcondsum=SPcondsum+squeeze(sum(spvalues2(:,:,i,1:stimsumnum(i)),4));
                SPcondnum=SPcondnum+stimsumnum(i);
                SPcondtemp(:,:,n:n+stimsumnum(i)-1)=squeeze(spvalues2(:,:,i,1:stimsumnum(i)));
                n=n+stimsumnum(i);
            end
            spavg2(:,:,j)=SPcondsum/SPcondnum;
            spstd2(:,:,j)=std(SPcondtemp, 0, 3);
            stimsumnum(j)=SPcondnum;
            clear SPcondtemp;
        end
    end
    spavg2=reshape(spavg2, [framenum, masknum, NStim+NSPcond]);
    spstd2=reshape(spstd2, [framenum, masknum, NStim+NSPcond]);

    % superpix1 (same mask are grouped together)
    fprintf(superfidb1, 'dR/R value\t');
    fprintf(superfidb1, '%f\t', xlabel);
    fprintf(superfidb1, '\r');
    for (j=1:masknum)
        fprintf(superfidb1, '%s\t', spmaskname(j, :));
        fprintf(superfidb1, 'f%d\t', 1:FramesPerStim);
        if flagsaveframeave
            fprintf(superfida1, 'fframe\t');
            fprintf(superfida1, 'sumrange\t');
        end
        ...
        for (i=1:NStim+NSPcond)
            if i<=NStim
                fprintf(superfidb1, 'stim%d\t', i);
            else
                fprintf(superfidb1, '%s\t', getfield(cell2struct(SPcond(i-NStim, 1), 'junk'), 'junk'));
            end
            for (f=1:framenum)
                fprintf(superfidb1, '%3.8f\t', spavg2(f, j, i));			% original value
            end
            fprintf(superfidb1, '\r');
        end
    end
    fprintf(superfidb1, '\r\r_____SD_____\r\t');
    fprintf(superfidb1, '%f\t', xlabel);
    fprintf(superfidb1, '\r');
    for (j=1:masknum)
        fprintf(superfidb1, '%s\t', spmaskname(j, :));
        ...
        for (i=1:NStim+NSPcond)
            ...
            for (f=1:framenum)
                if blockselectnum>1
	                fprintf(superfidb1, '%3.8f\t', spstd2(f, j, i));
	            end
            end
            fprintf(superfidb1, '\r');
        end
    end
    fprintf(superfidb1, '\r\r_____Num_____\r\t');
    fprintf(superfidb1, '%f\t', xlabel);
    for (j=1:masknum)
        fprintf(superfidb1, '%s\t', spmaskname(j, :));
        fprintf(superfidb1, 'f%d\t', 1:FramesPerStim);
        ...
        for (i=1:NStim+NSPcond)
            ...
            for (f=1:framenum)
                fprintf(superfidb1, '%d\t', stimsumnum(i));
            end
            fprintf(superfidb1, '\r');
        end
    end
    fprintf(superfidb1, '\r\r_____SEM_____\r\t');
    fprintf(superfidb1, '%f\t', xlabel);
    for (j=1:masknum)
        fprintf(superfidb1, '%s\t', spmaskname(j, :));
        fprintf(superfidb1, 'f%d\t', 1:FramesPerStim);
        ...
        for (i=1:NStim+NSPcond)
            ...
            for (f=1:framenum)
                fprintf(superfidb1, '%3.8f\t', spstd2(f, j, i)/sqrt(stimsumnum(i)));
            end
            fprintf(superfidb1, '\r');
        end
    end
    ...
    fprintf(superfidb2, 'dR/R value\t');
    fprintf(superfidb2, '%f\t', xlabel);
    fprintf(superfidb2, '\r');
    for (i=1:NStim+NSPcond)
        if i<=NStim
            fprintf(superfidb2, 'stim%d\t', i);
        else
            fprintf(superfidb2, '%s\t', getfield(cell2struct(SPcond(i-NStim, 1), 'junk'), 'junk'));
        end
        fprintf(superfidb2, 'f%d\t', 1:FramesPerStim);
        if flagsaveframeave
            fprintf(superfida1, 'fframe\t');
            fprintf(superfida1, 'sumrange\t');
        end
        fprintf(superfidb2, '\r');
	    for (j=1:masknum)
	        fprintf(superfidb2, '%s\t', spmaskname(j, :));
            for (f=1:framenum)
                fprintf(superfidb2, '%3.8f\t', spavg2(f, j, i));			% original value
            end
            fprintf(superfidb2, '\r');
        end
    end
    fprintf(superfidb2, '\r\r_____SD_____\r\t');
    fprintf(superfidb2, '%f\t', xlabel);
    fprintf(superfidb2, '\r');
    for (i=1:NStim+NSPcond)
        if i<=NStim
        ...
        for (j=1:masknum)
            ...
            for (f=1:framenum)
                if blockselectnum>1
	                fprintf(superfidb2, '%3.8f\t', spstd2(f, j, i));
	            end
            end
            fprintf(superfidb2, '\r');
        end
    end
    fprintf(superfidb2, '\r\r_____Num_____\r\t');
    fprintf(superfidb2, '%f\t', xlabel);
    fprintf(superfidb2, '\r');
    for (i=1:NStim+NSPcond)
        if i<=NStim
        ...
        for (j=1:masknum)
            ...
            for (f=1:framenum)
                if blockselectnum>1
	                fprintf(superfidb2, '%d\t', stimsumnum(i));
	            end
            end
            fprintf(superfida2, '\r');
        end
    end
    fprintf(superfidb2, '\r\r_____SEM_____\r\t');
    fprintf(superfidb2, '%f\t', xlabel);
    fprintf(superfidb2, '\r');
    for (i=1:NStim+NSPcond)
        if i<=NStim
        ...
        for (j=1:masknum)
            ...
            for (f=1:framenum)
                if blockselectnum>1
	                fprintf(superfidb2, '%3.8f\t', spstd2(f, j, i)/sqrt(stimsumnum(i)));
	            end
            end
            fprintf(superfidb2, '\r');
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Super Pixel time course raw data output, only for 'flagrawdatasp>0' and 'flagspmask>0'
if (flagsavespdataall>0)
    if flagsavespvalues==1 && flagloadspvalues~=1
        save(strcat(resultfolder, 'spvalues.mat'), 'spvalues');
        save(strcat(resultfolder, 'stimsumnum.mat'), 'stimsumnum');
    end
    ...
    if ~isdir([resultfolder, 'superpixel_data_all'])
        mkdir(resultfolder, 'superpixel_data_all');    
    end
    xlabel=(1:FramesPerStim)/FrameRate;     % in unit of second
    for (j=1:masknum)
        superfida3= fopen(strcat(resultfolder, strcat('superpixel_data_all\', 'superpixel_raw_data_all_', spmaskname(j, :), '.txt')), 'w'); 
%         superfida3= fopen(strcat(resultfolder, strcat('superpixel_raw_all.txt')), 'w'); 
        fprintf(superfida3, 'raw value\t');
        fprintf(superfida3, '%f\t', xlabel);
        fprintf(superfida3, '\r');
        for (i=1:NStim)
            fprintf(superfida3, 'stim%d\t', i);
            fprintf(superfida3, 'f%d\t', 1:FramesPerStim);
            if flagsaveframeave
                fprintf(superfida1, 'fframe\t');
                fprintf(superfida1, 'sumrange\t');
            end
%             fprintf(superfida3, 'gs%d\t');
            fprintf(superfida3, '\r');
            counter=0;
            for k=blockselect  % start block by block processing
                fprintf(superfida3, 'block%d\t', k);
                if goodstim(k, i)~=0
                    counter=counter+1;
                    for (f=1:framenum)
                        fprintf(superfida3, '%6.5f\t', spvalues(f, j, i, counter));			% original value
                    end
%                     fprintf(superfida3, '%d', 1);
                else
                    for (f=1:framenum)
                        fprintf(superfida3, '\t');
                    end
%                     fprintf(superfida3, '%d', 0);
                end
                fprintf(superfida3, '\r');
            end  
            fprintf(superfida3, '\r');
    ...
    for (j=1:masknum)
        superfida3= fopen(strcat(resultfolder, strcat('superpixel_data_all\', 'superpixel_dRR_data_all_', spmaskname(j, :), '.txt')), 'w');
%         superfida3= fopen(strcat(resultfolder, strcat('superpixel_raw_all.txt')), 'w'); 
        fprintf(superfida3, 'raw value\t');
        fprintf(superfida3, '%f\t', xlabel);
        fprintf(superfida3, '\r');
...
%             fprintf(superfida3, 'gs%d\t');
            ...
            for k=blockselect  % start block by block processing
                ...
                if goodstim(k, i)~=0
                    counter=counter+1;
                    for (f=1:framenum)
                        fprintf(superfida3, '%6.5f\t', spvalues2(f, j, i, counter));			% original value
                    end
%                     fprintf(superfida3, '%d', 1);
                else
                    for (f=1:framenum)
...
if flagspttest1==1 && flagsp>0
    NSPttestcond1=size(SPttestcond1, 1);
    for j=1:NSPttestcond1
        condlist1=getfield(cell2struct(SPttestcond1(j, 2), 'junk'), 'junk');
        condlist2=getfield(cell2struct(SPttestcond1(j, 3), 'junk'), 'junk');
        n=1;
        m=1;
        for i=condlist1
        ...
        [H,p,CI,STATS] = ttest2(SPttesttemp1, SPttesttemp2, 0.05, tail, 'equal', 3);
        p2(:,:,j)=p;
        clear SPttesttemp1;
        clear SPttesttemp2;
    end
    ...
    superfidt1= fopen(strcat(resultfolder, strcat('superpixel_ttest1_raw1', '.txt')), 'w');  % for sp avg & stdev output, 1, 2 are two different formats
    superfidt2= fopen(strcat(resultfolder, strcat('superpixel_ttest1_raw2', '.txt')), 'w');    

    % superpix_original1
    fprintf(superfidt1, 'original value\t');
    fprintf(superfidt1, '%f\t', xlabel);
    fprintf(superfidt1, 'fframe\t');
    fprintf(superfidt1, 'sumrange\t');
    fprintf(superfidt1, '\r');
    for (j=1:masknum)
        fprintf(superfidt1, '%s\t', spmaskname(j, :));
        fprintf(superfidt1, 'f%d\t', 1:FramesPerStim);
        fprintf(superfidt1, '\r');
        for (i=1:NSPttestcond1)
            fprintf(superfidt1, '%s\t', getfield(cell2struct(SPttestcond1(i, 1), 'junk'), 'junk'));
            for (f=1:framenum)
                fprintf(superfidt1, '%1.8f\t', p2(f, j, i));			% original value
            end
            fprintf(superfidt1, '\r');
        end
    end
    ...
    % superpix_original2 is just another format for sp1 (same stim are grouped together) %added by Hisashi 
    fprintf(superfidt2, 'original value\r\t');
    fprintf(superfidt2, '%f\t', xlabel);
    fprintf(superfidt2, 'fframe\t');
    fprintf(superfidt2, 'sumrange\t');
    fprintf(superfidt2, '\r');
    for (i=1:NSPttestcond1)
        fprintf(superfidt2, '%s\t', getfield(cell2struct(SPttestcond1(i, 1), 'junk'), 'junk'));
        fprintf(superfidt2, 'f%d\t', 1:FramesPerStim);
        fprintf(superfidt2, '\r');
        for (j=1:masknum)
            fprintf(superfidt2, '%s\t', spmaskname(j, :));
            for (f=1:framenum)
                fprintf(superfidt2, '%1.8f\t', p2(f, j, i));			% original value
            end
            fprintf(superfidt2, '\r');
        end
    end
    ...
    for j=1:NSPttestcond1
        ...
        m=1;
        for i=condlist1
            SPttesttemp1(:,:,n:n+stimsumnum(i)-1)=squeeze(spvalues2(:,:,i,1:stimsumnum(i)));
            n=n+stimsumnum(i);
        end
        for i=condlist2
            SPttesttemp2(:,:,m:m+stimsumnum(i)-1)=squeeze(spvalues2(:,:,i,1:stimsumnum(i)));
            m=m+stimsumnum(i);
        end
        [H,p,CI,STATS] = ttest2(SPttesttemp1, SPttesttemp2, 0.05, tail, 'equal', 3);
        p2(:,:,j)=p;   
    ...
    superfidu1= fopen(strcat(resultfolder, strcat('superpixel_ttest1_dRR1', '.txt')), 'w');  % for sp avg & stdev output, 1, 2 are two different formats
    superfidu2= fopen(strcat(resultfolder, strcat('superpixel_ttest1_dRR2', '.txt')), 'w');

    % superpix2 is just another format for sp1 (same stim are grouped together)
    fprintf(superfidu1, 'dR/R value\t');
    fprintf(superfidu1, '%f\t', xlabel);
    fprintf(superfidu1, 'fframe\t');
    fprintf(superfidu1, 'sumrange\t');
    fprintf(superfidu1, '\r');
    for (j=1:masknum)
        fprintf(superfidu1, '%s\t', spmaskname(j, :));
        fprintf(superfidu1, 'f%d\t', 1:FramesPerStim);
        fprintf(superfidu1, '\r');
        for (i=1:NSPttestcond1)
            fprintf(superfidu1, '%s\t', getfield(cell2struct(SPttestcond1(i, 1), 'junk'), 'junk'));
            for (f=1:framenum)
                fprintf(superfidu1, '%1.8f\t', p2(f, j, i));			% original value
            end
            fprintf(superfidu1, '\r');
        end
    end
    ...
    % superpix_original2 is just another format for sp1 (same stim are grouped together) %added by Hisashi 
    fprintf(superfidu2, 'dR/R value\t');
    fprintf(superfidu2, '%f\t', xlabel);
    fprintf(superfidu2, 'fframe\t');
    fprintf(superfidu2, 'sumrange\t');
    fprintf(superfidu2, '\r');
    for (i=1:NSPttestcond1)
        fprintf(superfidu2, '%s\t', getfield(cell2struct(SPttestcond1(i, 1), 'junk'), 'junk'));
        fprintf(superfidu2, 'f%d\t', 1:FramesPerStim);
        fprintf(superfidu2, '\r');
        for (j=1:masknum)
            fprintf(superfidu2, '%s\t', spmaskname(j, :));
            for (f=1:framenum)
                fprintf(superfidu2, '%1.8f\t', p2(f, j, i));			% original value
            end
            fprintf(superfidu2, '\r');
        end
    end
...
if flagvector==1
    ...
    if ~isdir([resultfolder, vectorresultname])
        mkdir(resultfolder, vectorresultname);    
    end
	for i=1:Nvect
    	...
    	savefilename=strcat(resultfolder, vectorresultname, savefilename);
...
%             end
%             filename=strcat(resultfolder, vectorresultname, filename);
%             b=OIReadIVF(filename);
%     %         if (i==1)
%     %             [height, width]=size(b);
%     %             singlemap = zeros(height, width, Nvect); 
...
%	figure; image(norm_to_uint8(Polarmap.ang));  axis equal; axis off; colormap(lut);  colorbar; %%load billcolorfix; 
%	figure; imagesc(norm_to_uint8(Polarmap.mag)); axis equal; axis off; colormap(gray(256)); colorbar; 
	filename1 = strcat('Angle_', LPfiltermethod, '_', num2str(lowpass), '-', HPfiltermethod, '_', num2str(highpass), ext, '.tif');
    filename1 = strcat(resultfolder, vectorresultname, filename1);
	imwrite(norm_to_uint8(Polarmap.ang), lut, filename1, 'tiff');
%	OIWriteIVF(Polarmap.ang, [filename1(1:end-4), ext, '.ivf']);
	filename2 = strcat('Mag_', LPfiltermethod, '_', num2str(lowpass), '-', HPfiltermethod, '_', num2str(highpass), ext, '.tif');
    filename2 = strcat(resultfolder, vectorresultname, filename2);
	imwrite(norm_to_uint8(OIClipH(Polarmap.mag, clipmethod, clipvalue)), filename2, 'tiff');
    if flagsaveivfvector
        OIWriteIVF(Polarmap.mag, [filename2(1:end-4), ext, '.ivf']);
    end
	filename3 = strcat('Polar_', LPfiltermethod, '_', num2str(lowpass), '-', HPfiltermethod, '_', num2str(highpass), ext, '.tif');
    filename3 = strcat(resultfolder, vectorresultname, filename3);
    mag=norm_to_01(Polarmap.mag);
    mag=OIClipH(mag, 1, polarclipsd);   % to adjust map darkness  
    switch precisiontype
        case 'single'
...
%	OIWriteIVF(polarmap, [filename3(1:end-4), ext, '.ivf']);
	filename4 = strcat('Sum', num2str(lowpass), '-', num2str(highpass), HPfiltermethod, ext, '.tif');
    filename4 = strcat(resultfolder, vectorresultname, filename4);
	imwrite(norm_to_uint8(OIClipH(Polarmap.sum, clipmethod, clipvalue)), filename4, 'tiff');
%	OIWriteIVF(Polarmap.sum, [filename4(1:end-4), ext, '.ivf']);
    ...
    imwrite(uint8(colorbarmap), strcat(resultfolder, vectorresultname, 'colortable.tif'), 'tiff'); 
...
fprintf('\n\n\nTotal time used: %8.4f secs (%8.4f minutes).\r\r', totaltime, totaltime/60);
