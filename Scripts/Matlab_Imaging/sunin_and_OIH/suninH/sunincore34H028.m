...
fprintf('\rSelected %d blk files.\r', blockselectnum);
...
if (flagloadspvalues~=1)
    for k=blockselect  % start block by block processing
        ...
        for i = 1:NStim
          if goodstim(k, i)==0    % skip this stim; k: blockselect; i: NStim
...
%                         DCFrames2(:,:,f)=OIEasyFilterH2(DCFrames(:,:,f), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize); %modified by Hisashi  
%                      end
%                 else
%                     DCFrames2=DCFrames;
%                 end
...
%                 if flagsp==0 || (flagsp>0 && ~(flaghpfilter||flaglpfilter) && ~flagalign>0 )
%                     clear DCFrames2;
%                 end
%             end
...
% % 	    	for f=1:FramesPerStim
% % 	        	avgblock(:,:,f,i)=OIEasyFilterH2(avgblock(:,:,f,i), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize); %modified by Hisashi  
% % 	        end
% %         end
%     end 
...
%             end
            SNCFilterSMapH3; % modified by Hisashi
        end
    end
