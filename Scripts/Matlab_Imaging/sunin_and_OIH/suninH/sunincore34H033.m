...
%%      (121116) H3.3 Blank subtraction function was improved more in making subtraction maps.
%%      (110322) H3.2 Blank subtraction function was improved. 'flagblanksubtraction', 'flagSPblanksubtraction', and 'flagSPTtestblanksubtraction' were introduced.
%%      (091222) H3.1 Bandpass filtering using FFT available
...
%%      (090129) H2.8 The parameters, 'flagsaveaverageblk', 'datasource=3', and 'flagsptrialfilter' were omitted. They had been introduced since Ver H2.1 
...
%     vectmask=ones(504); % mask used for Bosking normalization
    ...
    if flagvectormaskfilter
        if ~isdir(strcat(resultdriver, outputfolder, expname, runname, 'masks\vectormask'))
            filtervectormaskname = strcat(resultdriver, outputfolder, expname, runname, 'masks\filtermask\default.bmp');
        else
...
if (flagloadspvalues~=1)
    ...
    %             if flagtrialfilter && (flaghpfilter||flaglpfilter) && datasource~=3 %added by Hisashi on 080229
                if flagtrialfilter && (flaghpfilter||flaglpfilter) %added by Hisashi on 080229
                    ...
                    if strcmp(lpfmethod, 'fft') || strcmp(hpfmethod, 'fft')
                        SumFrame(:,:,currentstimid)=ifft2(fft2(SumFrame(:,:,currentstimid)).*fftfilter);
                    end                   
                end  
    %             if flagalign==41 && datasource~=3 % Modified by Hisashi on 080229
                ...
                if (flaggoodstim==1)
					...
					for j=1:smapnum
						...
						if any(condlistSmap1 == currentstimid)
%                             j
%                             condlistSmap1
%                             currentstimid
%                             any(condlistSmap1 == currentstimid)
%                             BBB1=sum(sum(NewSum_Smap1(:,:,j)))
							NewSum_Smap1(:,:,j) = NewSum_Smap1(:,:,j) + SumFrame(:,:,currentstimid);
							counter_Smap1(j) = counter_Smap1(j) +1;
							NewSum_Smap1_current(:,:,j) = NewSum_Smap1_current(:,:,j) + SumFrame(:,:,currentstimid);
							counter_Smap1_current(j) = counter_Smap1_current(j) +1;
%                             pause
						end
						if any(condlistSmap2 == currentstimid)
%                             j
%                             condlistSmap2
%                             currentstimid
%                             any(condlistSmap2 == currentstimid)
%                             BBB2=sum(sum(NewSum_Smap1(:,:,j)))
							NewSum_Smap2(:,:,j) = NewSum_Smap2(:,:,j) + SumFrame(:,:,currentstimid);
							counter_Smap2(j) = counter_Smap2(j) +1;
							NewSum_Smap2_current(:,:,j) = NewSum_Smap2_current(:,:,j) + SumFrame(:,:,currentstimid);
...
%                             pause
						...
						if flagblanksubtraction % HT 121115
							...
							if any(condlistBSub1 == currentstimid)
								NewSum_Bsub1_current(:,:,j) = NewSum_Bsub1_current(:,:,j) + SumFrame(:,:,currentstimid);
								counter_Bsub1_current(j) = counter_Bsub1_current(j) +1;
							end
							if any(condlistBSub2 == currentstimid)
								NewSum_Bsub2_current(:,:,j) = NewSum_Bsub2_current(:,:,j) + SumFrame(:,:,currentstimid);
                    ...
                    if (flaghpfilter||flaglpfilter)
                        ...
                        for f=1:FramesPerStim
                            if  (spoperation==1 || spoperation==3)
                                ...
                                if strcmp(lpfmethod, 'fft') || strcmp(hpfmethod, 'fft')
                                    DCFrames(:,:,f)=ifft2(fft2(DCFrames(:,:,f)).*fftfilter);
                                end
                            end
                            if  (spoperation==2 || spoperation==3)
...
if (flagsp>0)
    ...
    SNCSPTimeCourseH2 % modified by Hisashi on 110302, H32
end
