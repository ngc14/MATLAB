%% Sunincore	Optical Imaging Data Processing 	- HDL and HT
%%
%%      See 'Sunin_readme.txt' for detail useage. 

%%  	Ver 3.4.2.1 (150224) sumtype = 3 was introduced. SNCSPTimeCourseH3 was adopted. Hisashi
%%  	Ver 3.4.1 (150222) Frame-by-frame processes were improved. "flagmaskfilter" was omitted. Hisashi
%%  	Ver 3.4 (150221) flagalign = 2 now perfroms image shift frame by frame. need .4 file. dalsalineremove was introduced. Hisashi
...
%%      Ver 3.3 (121116) Blank subtraction function was improved more in making subtraction maps.
%%      Ver 3.3 (121116) Blank subtraction function was improved more in making subtraction maps.
%%      Ver 3.2 (110322) Blank subtraction function was improved. 'flagblanksubtraction', 'flagSPblanksubtraction', and 'flagSPTtestblanksubtraction' were introduced.
%%      Ver 3.1 (091222) Bandpass filtering using FFT available
%%      Ver 3.0 (090807) SNCSaveMapH was fixed. It now clips images correctly at center value +- range
%%                    Added 'circular' and 'replicate' for PADOPT in medfilt2 (medfilt2H). The default is 'symmetric'. 091011 
%%      Ver 2.9 (090207-090805)  Parameter 'stimselect' was introduced to select stims. This will reduce data reading time if you don't need to read all stim data.
%%            		  Parameter 'gsfilename' was introduced to specify goodstim file.
%%                    Parameter 'outputdriver' was introduced to determine the output folder flexibly.
%%      Ver 2.8 (090129) The parameters, 'flagsaveaverageblk', 'datasource=3', and 'flagsptrialfilter' were omitted. They had been introduced since Ver H2.1 
...
%%                     was introduced.
%%      Ver 2.7 (081014) 'flagsavespdataall' was introduced for save trial-by-trial raw data in a text file     
%%                                  'flagsaveframeave' was introduced
%%      Ver 2.5 (080502) 'flagvectormaskfilter' was introduced.
%%                                  SNCFilterSMapH2.m was introduced.
%%      Ver 2.3 (080303) Superpixel analysis part was updated and t-test function was introduced                  
%%      Ver 2.2 (080303) The default input and output folder name, 'Expt' and 'expt1' was changed to 'expt0' and 'expt1', respectively.
%%      Ver 2.1 (080228) You can averaged block file for analysis which you saved previously by 'flagsaveaverageblk=1'. 
%%                          The extention of this type of file is '.aBLK'. Set 'datasource=3'.
%%      Ver 2.0 (080130) You can choose precision type (precisiontype='single' or 'double') . Using 'single' precision may reduce memory usage.
%%      Ver 1.1 (080115) Add mask filtering function in SNCFilterSMapH.m (flagmaskfilter)
%%      Ver 1.0 (070502) Add trial by trial filtering (flagtrialfilter)
%%      Since here, Sunincore34H versions were made by Hisashi, based on Haidong’s V3.4.
...
% if exist ('flagvectormaskfilter','var')==0
%     flagvectormaskfilter=flagmaskfilter;
% end
if exist ('stimselect','var')==0
    stimselect=[]; %select stims for processing, eg. [1 3 4 5] or [1:5], leave empty for all block being select
end
if exist ('gsfilename','var')==0
...
%     end
    if flagmap==2 && ~isdir([resultfolder, 'singlecond\'])
        mkdir([resultfolder], 'singlecond\');    
    end
...
% if flagalign~=0 && flagalign~=41 % 150221 HT
...
%     vectmask=ones(504); % mask used for Bosking normalization
    ...
    if flagvectormask
        filtervectormaskname = strcat(resultdriver, outputfolder, expname, runname, 'masks\vectormask\default.bmp');
...
if flagvector==2
	...
	Polarmap=OIPolarH2(singlemap, lut, vectmask, polarclipmethod, polarclipvalue, lpfmethod, lpkernelsize, hpfmethod, hpkernelsize);
...
clipmaskname = strcat(resultdriver, outputfolder, expname, runname, 'masks\clipmask\default.bmp');     % can use a blood vessel map for clip masking, only for clipmethod==2
...
%     ivffolder=[resultfolder, 'blkavg\singlecond\'];
    ivffolder=[resultfolder, 'singlecond\'];
...
if strcmp(lpfmethod, 'fft') || strcmp(hpfmethod, 'fft')
    if FrameWidth == FrameHeight
        fftfilter=OIMakFIRfilterH(FrameWidth,lpfmethod, lpkernelsize, hpfmethod, hpkernelsize);
    else
        fprintf('Error: For FFT filtering, maps should be square.\n');
    end
else
    fftfilter= ones(FrameWidth);
end
...
%     maskfolder=strcat(resultdriver, outputfolder, expname, 'masks\spmask\');
    maskfolder=strcat(resultdriver, outputfolder, expname, runname, 'masks\spmask\') %100409 HT
    switch flagspmask
...
%         end                
        for i=1:masknum
            domainfidx=fopen (strcat(resultdriver, outputfolder, expname, runname, 'masks\spmask\', spmaskname(i,:), '_x.txt'), 'r'); %Hisashi
            domainfidy=fopen (strcat(resultdriver, outputfolder, expname, runname, 'masks\spmask\', spmaskname(i,:), '_y.txt'), 'r'); %Hisashi
            ...
            if (savemaskmap==1)     % output memory masks for evaluating if selected 
                maskmap=mask(:,:,i)*255;
                imwrite(uint8(maskmap), strcat(resultdriver, outputfolder, expname, runname, 'masks\spmask_', spmaskname(i,:), ext, '.bmp'), 'bmp');
            end
        end
        clear maskmap;
    end
...
if flagsp % 150223 HT
	FramesforAnalysis=[1:FramesPerStim];
else
	FramesforAnalysis=[fframe sumrange];
end
...
%                 if getfield(cell2struct(filename(k), 'junk'), 'junk')~=shiftstructname((k-1)*NStim*FramesPerStim+(i-1)*FramesPerStim+j).sfname
%                     fprintf('\rwrong file name match: %s vs %s', getfield(cell2struct(filename(k), 'junk'), 'junk'), shiftstructname((k-1)*NStim*FramesPerStim+(i-1)*FramesPerStim+j).sfname);
%                 end
%             end
%         end
%     end
...
if (flagalign==1)  % 41; Load shift parameters generated by flagalign==1 process (include '.1' file for all first frame shift and '.3' file for all rest frames)
    ...
    if size(sfname,1)~=blockfilenum*NStim
        fprintf('Error: flagalign===1, .2 file doesnot contain blockfilenum*NStim number of entries!\n');
        size(sfname,1)
        blockfilenum
        NStim
    end
    ...
    for k=1:blockselectnum
        for i=1:NStim
            if getfield(cell2struct(filename(k), 'junk'), 'junk')~=shiftstructname((k-1)*NStim+i).sfname
                fprintf('\wrong file name match: %s vs %s', getfield(cell2struct(filename(k), 'junk'), 'junk'), shiftstructname(k*NStim*FramesPerStim).sfname);
            end
        end
    end
elseif (flagalign==2)  % Load shift parameters generated by flagalign==2 process (need '.4' file for shift of all frames. Since H34)
    ...
    for k=1:blockselectnum
        for i=1:NStim
        	for j=1:FramesPerStim
                if getfield(cell2struct(filename(k), 'junk'), 'junk')~=shiftstructname((k-1)*NStim*(FramesPerStim-1)+(i-1)*(FramesPerStim-1)+j).sfname
                    fprintf('\rwrong file name match: %s vs %s', getfield(cell2struct(filename(k), 'junk'), 'junk'), shiftstructname((k-1)*NStim*(FramesPerStim-1)+(i-1)*(FramesPerStim-1)+j).sfname);
                end
            end
        end
    end
...
%     if flagmaskfilter
%         fprintf('\rMask will be applied for spatial filtering.');
%     end
    if flagtrialfilter
        fprintf('\rSpatial filtering will be applied trial-by-trial.');
    end
    if (flagsp && (lpkernelsize || hpkernelsize))
...
counter_Smap1=zeros(smapnum);
counter_Smap2=zeros(smapnum);
...
switch precisiontype %by Hisashi
    case 'single'
        ...
        if (flaggoodstim==1)
			...
			NewSum_Smap1=zeros(FrameHeight,FrameWidth,smapnum, 'single');
			NewSum_Smap2=zeros(FrameHeight,FrameWidth,smapnum, 'single');
        end
...
%         stimsumnum=zeros(NStim,1);	%keep how many stim was summed. (in randomized & goodstim cases, it may not same for each condition).
    ...
    case 'double'
        ...
        if (flaggoodstim==1)
			...
			NewSum_Smap1=zeros(FrameHeight,FrameWidth,smapnum);
			NewSum_Smap2=zeros(FrameHeight,FrameWidth,smapnum);
        end
 %         stimsumnum=zeros(NStim,1);	%keep how many stim was summed. (in randomized & goodstim cases, it may not same for each condition).
        stimsumnum=zeros(NStim+NSPcond,1);	%keep how many stim was summed. (in randomized & goodstim cases, it may not same for each condition).
        if flagsp
...
if (flagloadspvalues~=1)
    for k=blockselect  % start block by block processing
		...
		counter_Smap1_current=zeros(smapnum);
		counter_Smap2_current=zeros(smapnum);
        switch precisiontype %by Hisashi
            case 'single'
		        SumFrame=zeros([FrameHeight, FrameWidth, NStim], 'single');
                NewSum_Smap1_current=zeros(FrameHeight,FrameWidth,smapnum, 'single');
                NewSum_Smap2_current=zeros(FrameHeight,FrameWidth,smapnum, 'single');
            case 'double'
		        SumFrame=zeros([FrameHeight, FrameWidth, NStim]);
                NewSum_Smap1_current=zeros(FrameHeight,FrameWidth,smapnum);
                NewSum_Smap2_current=zeros(FrameHeight,FrameWidth,smapnum);
        end
		if (flaggoodstim==1) && flagblanksubtraction
			counter_Bsub1_current=zeros(smapnum);
			counter_Bsub2_current=zeros(smapnum);
			switch precisiontype %by Hisashi
				case 'single'
					NewSum_Bsub1_current=zeros(FrameHeight,FrameWidth,smapnum, 'single');
					NewSum_Bsub2_current=zeros(FrameHeight,FrameWidth,smapnum, 'single');
				case 'double'
					NewSum_Bsub1_current=zeros(FrameHeight,FrameWidth,smapnum);
					NewSum_Bsub2_current=zeros(FrameHeight,FrameWidth,smapnum);
			end
		end
        ...
        for i = 1:NStim
            if goodstim(k, i)==0 || sum(find(stimselect==i))==0  % skip this stim; k: blockselect; i: NStim. Since H029, skip stims unselected by 'stimselect'
                ...
                if flagrandom   % if stim seq in data is random, use i for reading, and use 'stimloc' for averageing
                    stimloc=stimseq(k, i);
                else
                    stimloc=i;
                end
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% remove artefact lines in images captured by Dalsa 1M60P 
				if dalsalineremove == 1
					for j = FramesforAnalysis  % 150221 HT
						 tDCFrames(:,:,j) = OIDalsalineremover(tDCFrames(:,:,j));
					end
				elseif dalsalineremove == 2
					for j = FramesforAnalysis  % 150221 HT
						 tDCFrames(:,:,j) = OIEasyFilterH2(tDCFrames(:,:,j), lpfmethod, 3, hpfmethod, 0);
					end
				end
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Image alignment 
    %             if datasource~=3
...
%                     end
                case 1 % 41; elseif (flagalign==41) % loading .2 shift parameter file and do shifting
                    DCFrames = tDCFrames;   % how much time will it cost for this operation? % commented out by HT 150221
                    % wait until later to do shifting
                case 2 %elseif (flagalign==43) % loading .4 shift parameter file and do shifting frame by frame % 150221 HT
                    for fr = FramesforAnalysis  % 150221 HT, for shift image frame by frame
                         DCFrames(:,:,fr) = OIShift(tDCFrames(:,:,fr), sfx3((k-1)*NStim*FramesPerStim+(stimloc-1)*FramesPerStim+fr), sfy3((k-1)*NStim*FramesPerStim+(stimloc-1)*FramesPerStim+fr));
                    end
                end
    %             else
    ...
    %             end
                ...
                case 4      % 4: dR/R0end: Devide by the average of first and last frames, see Chen-Bee__Frostig 1996
                    SumFrame(:,:,stimloc)=(tSumFrame-mean(DCFrames(:,:,fframe),3))./((mean(DCFrames(:,:,fframe),3)+DCFrames(:,:,FramesPerStim))/2);    
                otherwise
                    fprintf('\n  Error: need specify "operation" type!\n');
                    return
                end
    %             if flagtrialfilter && (lpkernelsize || hpkernelsize) && datasource~=3 %added by Hisashi on 080229
                if flagtrialfilter && (lpkernelsize||hpkernelsize) %added by Hisashi on 080229, modified on 150222
                    SumFrame(:,:,stimloc)=OIEasyFilterH2(SumFrame(:,:,stimloc), lpfmethod, lpkernelsize, hpfmethod, hpkernelsize); %modified by Hisashi	
                    if strcmp(lpfmethod, 'fft') || strcmp(hpfmethod, 'fft')
                        SumFrame(:,:,stimloc)=ifft2(fft2(SumFrame(:,:,stimloc)).*fftfilter);
                    end                   
                end  
                if flagalign==1; % 41 % Modified by Hisashi on 080229
                    SumFrame(:,:,stimloc)=OIShift(SumFrame(:,:,stimloc), sfx1((k-1)*NStim+i), sfy1((k-1)*NStim+i));
                end

                if (flaggoodstim==1)
                    NewSum(:,:,stimloc)=NewSum(:,:,stimloc)+SumFrame(:,:,stimloc);    % Add this sum because of use of 'goodstim'
					
					for j=1:smapnum
						condlistSmap1=getfield(cell2struct(Smap(j, 2), 'junk'), 'junk');
						condlistSmap2=getfield(cell2struct(Smap(j, 3), 'junk'), 'junk');
						if any(condlistSmap1 == stimloc)
%                             j
%                             condlistSmap1
%                             stimloc
%                             any(condlistSmap1 == stimloc)
%                             BBB1=sum(sum(NewSum_Smap1(:,:,j)))
							NewSum_Smap1(:,:,j) = NewSum_Smap1(:,:,j) + SumFrame(:,:,stimloc);
							counter_Smap1(j) = counter_Smap1(j) +1;
							NewSum_Smap1_current(:,:,j) = NewSum_Smap1_current(:,:,j) + SumFrame(:,:,stimloc);
							counter_Smap1_current(j) = counter_Smap1_current(j) +1;
%                             pause
						end
						if any(condlistSmap2 == stimloc)
%                             j
%                             condlistSmap2
%                             stimloc
%                             any(condlistSmap2 == stimloc)
%                             BBB2=sum(sum(NewSum_Smap1(:,:,j)))
							NewSum_Smap2(:,:,j) = NewSum_Smap2(:,:,j) + SumFrame(:,:,stimloc);
							counter_Smap2(j) = counter_Smap2(j) +1;
							NewSum_Smap2_current(:,:,j) = NewSum_Smap2_current(:,:,j) + SumFrame(:,:,stimloc);
							counter_Smap2_current(j) = counter_Smap2_current(j) +1;
%                             pause
						end
						if flagblanksubtraction % HT 121115
							condlistBSub1=getfield(cell2struct(SmapBSub(j, 2), 'junk'), 'junk');
							condlistBSub2=getfield(cell2struct(SmapBSub(j, 3), 'junk'), 'junk');
							if any(condlistBSub1 == stimloc)
								NewSum_Bsub1_current(:,:,j) = NewSum_Bsub1_current(:,:,j) + SumFrame(:,:,stimloc);
								counter_Bsub1_current(j) = counter_Bsub1_current(j) +1;
							end
							if any(condlistBSub2 == stimloc)
								NewSum_Bsub2_current(:,:,j) = NewSum_Bsub2_current(:,:,j) + SumFrame(:,:,stimloc);
								counter_Bsub2_current(j) = counter_Bsub2_current(j) +1;
							end
						end
					end
                end
                stimsumnum(stimloc)=stimsumnum(stimloc)+1;     % record how many repeats for each stim
                ...
                if (flagsp>0)
    %                 if (lpkernelsize || hpkernelsize) && datasource~=3 %added by Hisashi on 080307
%                     if (lpkernelsize || hpkernelsize)
                    ...
                    for f=1:FramesPerStim
                        ...
                        if  (spoperation==2 || spoperation==3)
                            ...
                            if (lpkernelsize || hpkernelsize) % 150223 HT
                                ...
                                if strcmp(lpfmethod, 'fft') || strcmp(hpfmethod, 'fft')
                                    DCFrames3drr(:,:,f)=ifft2(fft2(DCFrames3drr(:,:,f)).*fftfilter);
                                end
                            end
                        end
                    end
...
%                         DCFrames3=DCFrames;
%                     end
                    if flagalign==1; %41 %added by Hisashi on 080307
                        for j = 1:FramesPerStim
                            if  ~(lpkernelsize || hpkernelsize) || ((lpkernelsize || hpkernelsize) && (spoperation==1 || spoperation==3))
                                DCFrames3(:,:,j) = OIShift(DCFrames3(:,:,j), sfx1((k-1)*NStim+i), sfy1((k-1)*NStim+i));
                            end
                            if (lpkernelsize || hpkernelsize) && (spoperation==2 || spoperation==3)
                                ...
                                if f<=FramesPerStim
                                    switch precisiontype
                                        case 'single'
                                            ...
                                            spvalues(f,j,stimloc,stimsumnum(stimloc))=sum(sum(maskedmap))/sum(sum(single(mask(:,:,j))));
                                        case 'double'
                                            maskedmap= double(DCFrames3(:,:,f)).*double(mask(:, :, j));
                                            spvalues(f,j,stimloc,stimsumnum(stimloc))=sum(sum(maskedmap))/sum(sum(double(mask(:,:,j))));
                                    end
                                elseif f==FramesPerStim+1
                                    spvalues(f,j,stimloc,stimsumnum(stimloc))=mean(squeeze(spvalues(fframe,j,stimloc,stimsumnum(stimloc))));
                                elseif f==FramesPerStim+2
                                    spvalues(f,j,stimloc,stimsumnum(stimloc))=mean(squeeze(spvalues(sumrange,j,stimloc,stimsumnum(stimloc))));
                                end
                            end
%                             if (lpkernelsize || hpkernelsize) && (spoperation==2 || spoperation==3)
                            if (spoperation==2 || spoperation==3)
                                if f<=FramesPerStim
                                    switch precisiontype
                                        case 'single'
                                            ...
                                            spvaluesdrr(f,j,stimloc,stimsumnum(stimloc))=sum(sum(maskedmap2))/sum(sum(single(mask(:,:,j))));
                                        case 'double'
                                            maskedmap2= double(DCFrames3drr(:,:,f)).*double(mask(:, :, j));
                                            spvaluesdrr(f,j,stimloc,stimsumnum(stimloc))=sum(sum(maskedmap2))/sum(sum(double(mask(:,:,j))));
                                    end
                                elseif f==FramesPerStim+1
                                    spvaluesdrr(f,j,stimloc,stimsumnum(stimloc))=mean(squeeze(spvaluesdrr(fframe,j,stimloc,stimsumnum(stimloc))));
                                elseif f==FramesPerStim+2
                                    spvaluesdrr(f,j,stimloc,stimsumnum(stimloc))=mean(squeeze(spvaluesdrr(sumrange,j,stimloc,stimsumnum(stimloc))));
                                end
                            end
                        end              
                    end
...
%                         clear DCFrames3drr;
%                     end
                    clear DCFrames3 DCFrames3drr; % 150223 HT
                end    %% (flagsp>0)
...
%         end
        ...
        if (flagmap~=0)
            for j=1:smapnum						% note: not differentiate goodstim=1 or 0, get single block map anyway
...
%                 j
				...
				if flagblanksubtraction % HT 121115
% 				flagblanksubtraction
					if counter_Bsub1_current(j)
						NewSum_Bsub1_current(:,:,j) = NewSum_Bsub1_current(:,:,j) / counter_Bsub1_current(j);
						NewSum_Smap1(:,:,j) = NewSum_Smap1(:,:,j) - NewSum_Bsub1_current(:,:,j) * counter_Smap1_current(j); %Blank subtraction
					else
						NewSum_Smap1(:,:,j) = NewSum_Smap1(:,:,j) - NewSum_Smap1_current(:,:,j);
						counter_Smap1(j) = counter_Smap1(j) - counter_Smap1_current(j);
					end
					if counter_Bsub2_current(j)
						NewSum_Bsub2_current(:,:,j) = NewSum_Bsub2_current(:,:,j) / counter_Bsub2_current(j);
						NewSum_Smap2(:,:,j) = NewSum_Smap2(:,:,j) - NewSum_Bsub2_current(:,:,j) * counter_Smap2_current(j); %Blank subtraction
					else
						NewSum_Smap2(:,:,j) = NewSum_Smap2(:,:,j) - NewSum_Smap2_current(:,:,j);
						counter_Smap2(j) = counter_Smap2(j) - counter_Smap2_current(j);
					end
				end
            end
%             fprintf('clear');
			clear NewSum_Smap1_current NewSum_Smap2_current counter_Smap1_current counter_Smap2_current NewSum_Bsub1_current NewSum_Bsub2_current counter_Bsub1_current counter_Bsub2_current
    ...
    %         else
            if (flagmap~=0 && flaggoodstim ~=1) || (flagvector~=0 && flaggoodstim ~=1)  || (flagmap==3) || (flagmap==4) ||(flagquantify~=0)
                for i = 1:mapnum   
                    maps(:,:,i) =  maps(:,:,i) + singlemap(:,:,i); % The variable 'maps' here will not be used if if (flaggoodstim==1) %121115 HT 
                    if tempsaveblocks~=0
...
if (flagmap~=0) || (flagvector~=0)     
    if (flaggoodstim==1)        % if use goodstim, avg map is from averaged single condition map,  if goodstim=0, use averaged subtraction map, should be the same. 
        ...
        if flagmap~=0
%             SNCCalculateSMapH; % modified by Hisashi on 110322, H32
            SNCCalculateSMapH2; % modified by Hisashi on 121115, H33
        end
    else 	%         
        for i=1:mapnum
...
if (flagsp>0)
    ...
    SNCSPTimeCourseH3 % modified by Hisashi on 150224 (H34); modified by Hisashi on 110302 (H32)
end
...
if (flagsp>0)
    if flagspttest==1 && flagsp>0
        ...
        SNCSPTtestH2 % modified by Hisashi on 110302, H32
    end
end
...
%             end
    ...
    SNCSaveSMapH2; % modified by Hisashi on 121118
end
