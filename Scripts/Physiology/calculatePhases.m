...
psthPhaseEnds = cellfun(@(a) cellfun(@(ps) cellfun(@(pa,pw) ...
    findBins(mean(ps(:,pa),max(1,keepTrials.*(keepTrials+length(size(ps)))),'omitnan')+...
    pw,bins),phaseAlignment,phaseWindows,'UniformOutput', false), a, 'UniformOutput', false),alignTimes,'UniformOutput', false);
...
if(AUCCalc)
    condPhases = cellfun(@(udh,u) cellfun(@(ps) cellfun(@(p)...
        permute(mean(u.*~isnan(permute(repmat(p,[1,1,size(u,1)]),[3 2 1])),2),[1 3 2]),...
        ps,'UniformOutput',false),udh,'UniformOutput',false),psthPhaseMask,psths,'UniformOutput', false);
else
     condPhases = cellfun(@(udh,ud) cellfun(@(ps,u) cellfun(@(p)...
        permute(mean(u.*~isnan(permute(repmat(p,[1,1,size(u,1)]),[3 2 1]))./...
        ~isnan(permute(repmat(p,[1,1,size(u,1)]),[3 2 1])),2,'omitnan'),[1 3 2]),ps,'UniformOutput',false),...
        udh,{ud},'UniformOutput',false),psthPhaseMask,psths,'UniformOutput', false);
end
...
phaseL = num2cell(max(cat(3,phaseL{:}),[],3),1); % [s(:,1),max(s(:,1),(s(:,2)-e))]
bBounds = cellfun(@(s) cellfun(@(e) [s(:,1),max(s(:,1)+(e*double(all(e==mode(e)))),s(:,2)*double(~all(e==mode(e))))],...
    phaseL,'UniformOutput',false),ap, 'UniformOutput',false);%bBounds = cellfun(@(a) cat(3,a{:}), bBounds, 'UniformOutput', false);
...
AUCPSTH = cellfun(@(m) {median(cat(3,m{:}),3,'omitnan')},num2cell(vertcat(nMed{:}),2),'UniformOutput',false)';
if(~trialMean)
