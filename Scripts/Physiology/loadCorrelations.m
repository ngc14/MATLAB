    ...
    if(length(spikes)>=2)
        ...
        for ti = 1:length(tRange)
            ...
            for u = 1:sum(taskUnits)
                for i = 1:sum(taskUnits)%abs
                    condCurrMat(u,i,:) = cell2mat(cellfun(@(cs) abs(corr(cs(u,:)',cs(i,:)',Rows='pairwise')), currSpks, 'UniformOutput',false));
                end
            end
        ...
        for u = 1:sum(taskUnits)
            ...
            for i = 1:sum(taskUnits)
                ...
                % ctm = {};
                % for t = 1:numTrials
                %     ctm{t} = cellfun(@(cp) c orr(cp(u,binInds(1):binInds(end),t)',...
