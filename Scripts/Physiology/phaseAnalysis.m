...
normBaseline = cellfun(@(p,t)cellfun(@(a,n) [max(1,median(cell2mat(reshape(cellfun(@(c,s) ...
    permute(mean(c(:,s:s+(1/params.binSize),:),[2],'omitnan'),[1 3 2]),squeeze(num2cell(a,[1 2])),num2cell([ones(1,all(isnan(n)));...
    n(~isnan(n))]),'UniformOutput',false),[1,1,max(1,sum(~isnan(n)))])),3,'omitnan'));],p,t,...
    'UniformOutput',false),siteTrialPSTHS,allCondSegs,"UniformOutput",false);
%%
normPSTH = cellfun(@(cp,nb) num2cell(cellfun(@(p,b)p./repmat(b,[ones(1,length(size(p))-1),size(p,3)]),...
    cp,nb,'UniformOutput',false),2),siteTrialPSTHS,normBaseline,'Uniformoutput', false);
%%
