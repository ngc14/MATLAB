function plotProj(loadings,exp,epochSegs,somaProjs,somaLabs,location,num_dims,conditions,timeBins,saveFig,savePath)
...
nexttile(tl1,[1,1]); hold on; title("Variance Explained");plot(cumsum(exp),'LineWidth',2);ylim([0 100]);xlim([0 length(somaLabs)]);
plot([0 length(somaLabs)],[90 90],'r--','LineWidth',1);xlim([0,200]);
bx=nexttile(tl1,[1 1]);
...
%%
statePlot=size(weightedPSTHS{1},1)==1;
for n = 1:3
    figure();
    lc = {'-','-'};ax = {};plotSegs={};
    if(n==1)
    ...
    for c = 1:length(conditions)
        for s =1:length(somaReps)
            ...
            weightedPSTHS = permute(reshape(weightedPSTHS,size(weightedPSTHS,1)/(max(1,size(weightedPSTHS,2))),[],size(weightedPSTHS,2)),[3 2 1]);
            if(n==1)
    ...
    if(~statePlot)
        cellfun(@(a,p) cellfun(@(x,s) plot(a,[x;x], repmat(get(a,'YLim'),size(x,2),1)','LineStyle',':','Color',s,'LineWidth',1.5),num2cell(cell2mat(...
            cellfun(@(pc) pc(:,end-1:end),epochSegs(p),'UniformOutput',false)),2)',colors.values(cellstr(conditions(p)))),ax,plotSegs);
        ...
        cellfun(@(a) set(a,'XLim',[0 length(weightedPSTHS)]),ax);
        cellfun(@(a) set(a,'XTick',0:50:length(weightedPSTHS)),ax);
