function psthJointxUnit(repsIn,unitIndsIn,siteUnitModsIn,trialSegs,bins,PSTHS,...
    maxFRs,unitLocationIn,activityInds,alignLim,saveDirPath,saveName,FRRange)
HISTALIGNMENT = 1;
...
phaseJointFig = plotJointPSTHS(bins,unitPSTHS,trialSegs,reps,siteInds,activityInds,...
    alignLim,FRLim);
saveFigures(phaseJointFig,strcat(saveDirPath,"PSTH\"),saveName,[]);
...
phaseHMFig = unitJointPSTH(bins,unitNormPSTHS,reps,sortValsNaN,...
    trialSegs,siteInds,activityInds,alignLim);
...
mlHMFig = unitJointPSTH(bins,unitNormPSTHS,reps,mlSort,trialSegs,...
    siteInds,activityInds,alignLim);
