typeNames = ["Both","Reach","Grasp","All"];
conds = ["E","L","P"];
sOrder = [3 1 2];
...
dir = ["ML","RC"];
cl = validatecolor(["#EDB120","#A2142F","#0072BD","#77AC30"],'multiple');
whiskerP = 10;
%%
close all;
monkeys = 1;
            ...
            for m = 1:monkeys
                ...
                for s = 1:length(typeNames)
                    if(s==length(typeNames))
                        ...
                        sInd = tInd & tPhys{:,("unitType_E")}==sOrder(mod(s-1,3)+1);
                    ...
                    title(sax(m,s),typeNames(mod(s-1,4)+1));
                    hold on;
                    if(0)
