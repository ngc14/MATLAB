function I=imgaussianA(I,sigma,siz)
...
if(sigma>0)
    ...
    elseif(ndims(I)==4)
        Hx=reshape(H,[length(H) 1 1 1]);
        Hy=reshape(H,[1 length(H) 1 1]);
        Hz=reshape(H,[1 1 length(H) 1]);
        Ht=reshape(H,[1 1 1 length(H)]);
        I=imfilter(imfilter(imfilter(imfilter(I,Hx, 'same' ,'replicate'),Hy, 'same' ,'replicate'),Hz, 'same' ,'replicate'),Ht, 'same' ,'replicate');
    else
