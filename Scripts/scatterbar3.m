function scatterbar3(X,Y,Z,width,cl,al)
...
for j=1:r,
    for k=1:c,
        if ~isnan(Z(j,k))
            drawbar(X(j,k),Y(j,k),Z(j,k),width/2,cl(j,k,:),al(j,k))
        end
    end
end
...
function drawbar(x,y,z,width,cl,alp)
...
if(~isempty(cl))
    set(h,'facecolor','flat','FaceVertexCData',squeeze(cl)');
else
