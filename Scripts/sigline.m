...
VA = containers.Map({-1,1}, {'top', 'bottom'});
if nargin==1
    y=gety(gca);
    lbl=[];
elseif nargin==2
...
hold on
xs=[xs(1);xs(2)];
plot(xs,[1;1]*y*1.15,'-r', 'LineWidth',2);
plot([1;1]*xs(1),[y*1.1,y*1.15],'-r', 'LineWidth',2);%left edge drop
plot([1;1]*xs(2),[y*1.1,y*1.15],'-r', 'LineWidth',2);%right edge drop
if lbl
    text(mean(xs), y*1.2, lbl, 'HorizontalAlignment', 'center', 'VerticalAlignment', VA(sign(y)))% the sig star sign
end
hold off
...
function y=gety(h)
    ...
    if isgraphics(h) 
        switch(get(h,'type'))
            case {'line','hggroup','patch','bar','errorbar'}
                y=max(get(h,'ydata'));
                return;
            otherwise
                ys=[];
