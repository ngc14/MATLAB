...
try
    ...
    if(any(isinf(pixelCounts)))
        pixelCounts(isinf(pixelCounts)) = 10.^find(isinf(pixelCounts)) .* pixelCounts(find(~isinf(pixelCounts),sum(isinf(pixelCounts))));
    end
    pixelCounts = (pixelCounts-min(pixelCounts))./(max(pixelCounts)-min(pixelCounts));
    pixelCounts(isnan(pixelCounts)) = 0;
   ...
   % nonZeroBins = (nonZeroBins - min(nonZeroBins))./(max(nonZeroBins) - min(nonZeroBins));
	...
	lastNonZeroBin = length(nonZeroBins);
	...
	if side(1) == 'R'
		...
		ym = pixelCounts(lastNonZeroBin);
	else
		xPeak = nonZeroBins(find(pixelCounts == yPeak, 1, 'first'));
		xm = nonZeroBins(firstNonZeroBin); % Point on far left side, at the end of the tail on the left.
		ym = pixelCounts(lastNonZeroBin);
	end
	...
	if side(1) == 'R'
		...
		startBin = find(nonZeroBins==xPeak) -1;
		if startBin < 1
			startBin = 1;
		end
	...
	if showPlot
		...
		x0 = nonZeroBins(indexOfMaxDistance);
