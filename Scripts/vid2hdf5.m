...
%% Parameters
% Path to input file
videoPath = '\\pitt\sni\Gharbawie\Lab\ngc14\Track_Videos\01_03_2018\1.avi';

% Path to output file
% savePath = '..\..\leap\data\examples\072212_163153.clip.h5';
savePath = '\\pitt\sni\Gharbawie\Lab\ngc14\Scripts\1.h5';
...
meanIm = im2single(rgb2gray(meanIm));
 meanIm = [zeros(1,size(meanIm,2)); meanIm; zeros(1,size(meanIm,2))];
 meanIm(:,end+1) = zeros(1,size(meanIm,1));meanIm(:,end+1) = zeros(1,size(meanIm,1));
 meanIm(:,end+1) = zeros(1,size(meanIm,1));meanIm(:,end+1) = zeros(1,size(meanIm,1));
 meanIm = imresize(meanIm,.5);
...
while ~done
    ...
    if size(I,2) == 659
        I(:,end+1) = zeros(1,size(I,1));I(:,end+1) = zeros(1,size(I,1));I(:,end+1) = zeros(1,size(I,1));I(:,end+1) = zeros(1,size(I,1));
    end
    I = imresize(I,.5);
