...
trialFR = cellfun(@(ct,cs,ta) cellfun(@(a,b) cell2mat(cellfun(@(m,tt) ...
    squeeze(mean(m(:,max(1,tt(1)):max(1,tt(end)),:),2,'omitnan').*(1*~all(isnan(tt)))),squeeze(num2cell(a{1},[1,2])),...
    cellfun(@(e) [cell2mat(e),repmat([NaN,NaN],isempty(cell2mat(e)),1)],num2cell([arrayfun(@(bb) find(isalmost(params.bins,bb,params.binSize/1.99),1),b{1}(:,ta(1)),...
    'UniformOutput',false),arrayfun(@(bb) find(isalmost(params.bins,bb,params.binSize/1.99),1),b{1}(:,ta(end)),...
    'UniformOutput',false)],2),'UniformOutput',false),'UniformOutput',false)'),...
    ct,cs,'UniformOutput',false), siteTrialPSTHS(1:end-1), siteSegs(1:end-1),...
...
for c =1:length(conditions)
    ...
    siteUnits = mapSites2Units(condUnitMapping,cellfun(@(c) string(datetime(c,'Format','MMMM_dd')),...
        siteDateMap.Date','UniformOutput',false));
    ...
    for s = 1:length(unitPSTHS)
        ...
        if(~isempty(currsiteUnits))            
            ...
            unitChannels = allMaps{s}(siteChannels{c}{s}(tUnits{s}));
            ...
            plotColors =cell2struct(num2cell(distinguishable_colors(length(currsiteUnits)),2),...
                arrayfun(@(d) [char(datetime(siteDateMap.Date{s},'Format','MMMMdd')),'_',char(d)],cell2mat(unitChannels),'UniformOutput',false));
